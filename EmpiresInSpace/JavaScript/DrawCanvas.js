...
(function (DrawCanvas) {
    var canvas;
    var context;
    DrawCanvas.FogCanvas = document.createElement("canvas");
    DrawCanvas.standardTileSize = 30;
    var backgroundSize = 600;
    DrawCanvas.isScrollXEnabled = true;
    DrawCanvas.isScrollYEnabled = true;
    DrawCanvas.ScreenUpdate = false;
    DrawCanvas.TooltipShown = false;
    ...
    function setCanvas(_canvas) {
        ...
        DrawCanvas.FogContext = DrawCanvas.FogCanvas.getContext('2d');
    }
    DrawCanvas.setCanvas = setCanvas;
    function getCanvas() {
        return canvas;
    }
    DrawCanvas.getCanvas = getCanvas;
    function initInterface() {
        var temp = getSize();
        screenSize.width = temp.width;
        screenSize.height = temp.height;
    }
    DrawCanvas.initInterface = initInterface;
    ...
    DrawCanvas.pixelToGrid = pixelToGrid;
    function pixelToStandardGrid(x, y) {
        var col = x;
        var row = y;
        col = Math.round(col / DrawCanvas.standardTileSize);
        row = Math.round(row / DrawCanvas.standardTileSize);
        return { row: row, col: col };
    }
    var fps = 0;
    ...
    DrawCanvas.drawByRequestAnim = drawByRequestAnim;
    function showFPS() {
    ...
    function draw(sourceX, sourceY, destinationX, destinationY) {
        ...
        */
        ...
        DrawCanvas.FogContext.clearRect(0, 0, DrawCanvas.FogCanvas.width, DrawCanvas.FogCanvas.height);
        var Brightness = mainObject.user.scanRangeBrightness / 100;
        DrawCanvas.FogContext.fillStyle = "rgba(255, 255, 255, " + Brightness + ")";
        DrawCanvas.FogContext.fillRect(0, 0, DrawCanvas.FogCanvas.width, DrawCanvas.FogCanvas.height);
        ...
        if (currentMap.correspondingArea.Id == -1) {
            ...
            context.drawImage(DrawCanvas.FogCanvas, 0, 0);
            //create a box containing more than the screen:
    ...
    function clearShipScanAreas(displayPositionX, displayPositionY) {
        ...
        DrawCanvas.FogContext.clearRect(displayPositionX, displayPositionY, currentMap.tileSize, currentMap.tileSize);
    ...
    function clearFogOfWar(displayPositionX, displayPositionY, dimension) {
        DrawCanvas.FogContext.beginPath();
        DrawCanvas.FogContext.clearRect(displayPositionX, displayPositionY, currentMap.tileSize * dimension, currentMap.tileSize * dimension);
        DrawCanvas.FogContext.closePath();
    }
    ...
    DrawCanvas.drawCanvasTooltip = drawCanvasTooltip;
    ...
    DrawCanvas.checkScrollEnabling = checkScrollEnabling;
    function checkSrollOutOfBounds() {
        ...
        if (DrawCanvas.isScrollXEnabled) {
        ...
        if (DrawCanvas.isScrollYEnabled) {
    ...
    DrawCanvas.checkSrollOutOfBounds = checkSrollOutOfBounds;
    ...
    function doResize() {
        ...
        DrawCanvas.FogCanvas.width = canvas.width;
        DrawCanvas.FogCanvas.height = canvas.height;
        //DrawInterface.checkMapSizeScroll();
        DrawInterface.drawAll();
    }
    DrawCanvas.doResize = doResize;
    function showContextMenu() {
        Helpers.Log('showContextMenu');
    }
    DrawCanvas.showContextMenu = showContextMenu;
    ...
    DrawCanvas.scrollToHomePosition = scrollToHomePosition;
    ...
    DrawCanvas.scrollToPosition = scrollToPosition;
    ...
    DrawCanvas.scrollMapToPosition = scrollMapToPosition;
    ...
    DrawCanvas.checkMapSizeScroll = checkMapSizeScroll;
    ...
    DrawCanvas.refreshNavigationBar = refreshNavigationBar;
    ...
    DrawCanvas.deselectObject = deselectObject;
    ...
    DrawCanvas.switchToArea = switchToArea;
    ...
    DrawCanvas.refreshMiddleInfoPanel = refreshMiddleInfoPanel;
    //shows the middle Area with the ships that are on the same tile as the selected ship
    ...
    DrawCanvas.clickOnColony = clickOnColony;
    DrawCanvas.ShipToFleetDrag = null;
    ...
    DrawCanvas.createShipDiv = createShipDiv;
    function DroppedShipOnShip(event, ui) {
        ...
        if (DrawCanvas.ShipToFleetDrag.Fleet != null)
            DrawCanvas.ShipToFleetDrag.Fleet.RemoveShip(DrawCanvas.ShipToFleetDrag);
        if (targetShip.Fleet != null) {
            targetShip.Fleet.AddShip(DrawCanvas.ShipToFleetDrag);
        }
        else {
            var NewFleet = Ships.MakeFleet(true, targetShip, DrawCanvas.ShipToFleetDrag);
        }
        refreshMiddleInfoPanel();
    }
    DrawCanvas.DroppedShipOnShip = DroppedShipOnShip;
    function DroppedShipOnPanel(event, ui) {
        ...
        if (DrawInterface.ShipToFleetDrag.Fleet)
            DrawInterface.ShipToFleetDrag.Fleet.RemoveShip(DrawCanvas.ShipToFleetDrag);
        refreshMiddleInfoPanel();
    }
    DrawCanvas.DroppedShipOnPanel = DroppedShipOnPanel;
    ...
    DrawCanvas.createSurfaceFieldDiv = createSurfaceFieldDiv;
    ...
    DrawCanvas.createGoodsTd = createGoodsTd;
    ...
    DrawCanvas.createGoodsDiv = createGoodsDiv;
    ...
    DrawCanvas.createGoodsImageDiv = createGoodsImageDiv;
    ...
    DrawCanvas.createBar = createBar;
    ...
    DrawCanvas.createTextBar = createTextBar;
    ...
    DrawCanvas.cargoHoldUsed = cargoHoldUsed;
    ...
    DrawCanvas.createCargoHoldBar = createCargoHoldBar;
    ...
    */
    ...
    DrawCanvas.refreshMainScreenStatistics = refreshMainScreenStatistics;
    ...
    DrawCanvas.tooltipGoods = tooltipGoods;
    ...
    DrawCanvas.refreshQuickInfoGoods = refreshQuickInfoGoods;
