var StarMapModule;
(function (StarMapModule) {
    var OwnerNeigboursInfo = /** @class */ (function () {
        function OwnerNeigboursInfo() {
            this.UpperLeft = 0;
            this.Upper = 0;
            this.UpperRight = 0;
            this.Left = 0;
        ...
        OwnerNeigboursInfo.prototype.toOwnershipId = function (ownerId) {
            if (ownerId == 0)
                return 0;
            var corners = 0;
            var edges = 0;
            var bitted = 0;
            if (this.UpperLeft == ownerId) {
                corners++;
                bitted += 128;
            }
            if (this.UpperRight == ownerId) {
                corners++;
                bitted += 64;
            }
            if (this.LowerLeft == ownerId) {
                corners++;
                bitted += 32;
            }
            if (this.LowerRight == ownerId) {
                corners++;
                bitted += 16;
            }
            ...
            if (edges == 2) {
                ...
                if (this.Left == ownerId && this.Right == ownerId)
                    return 55;
                if (this.Upper == ownerId && this.Lower == ownerId)
                    return 54;
                //edgers are adjacent
                //and the field between both edges does NOT belong to player
                if (this.Left == ownerId && this.Upper == ownerId && this.UpperLeft != ownerId)
                    return 91;
                if (this.Upper == ownerId && this.Right == ownerId && this.UpperRight != ownerId)
                    return 92;
                if (this.Right == ownerId && this.Lower == ownerId && this.LowerRight != ownerId)
                    return 93;
                if (this.Lower == ownerId && this.Left == ownerId && this.LowerLeft != ownerId)
                    return 90;
                if (this.Left == ownerId && this.Upper == ownerId)
                    return 51;
                if (this.Upper == ownerId && this.Right == ownerId)
                    return 52;
                if (this.Right == ownerId && this.Lower == ownerId)
                    return 53;
                if (this.Lower == ownerId && this.Left == ownerId)
                    return 50;
            }
            //three edges adjacent to own region, one edge with border, one or two possible corners
            if (edges == 3) {
                if (this.Left != ownerId) {
                    if (this.UpperRight != ownerId && this.LowerRight == ownerId)
                        return 87; // one corner
                    if (this.LowerRight != ownerId && this.UpperRight == ownerId)
                        return 82; // one corner
                    if (this.UpperRight != ownerId && this.LowerRight != ownerId)
                        return 103; // 2 corners         
                    return 42;
                }
                if (this.Upper != ownerId) {
                    if (this.LowerRight != ownerId && this.LowerLeft == ownerId)
                        return 84; // one corner
                    if (this.LowerLeft != ownerId && this.LowerRight == ownerId)
                        return 83; // one corner
                    if (this.LowerRight != ownerId && this.LowerLeft != ownerId)
                        return 102; // 2 corners 
                    return 40;
                }
                if (this.Right != ownerId) {
                    if (this.UpperLeft != ownerId && this.LowerLeft == ownerId)
                        return 80; // one corner
                    if (this.LowerLeft != ownerId && this.UpperLeft == ownerId)
                        return 85; // one corner
                    if (this.UpperLeft != ownerId && this.LowerLeft != ownerId)
                        return 101; // 2 corners 
                    return 43;
                }
                if (this.Lower != ownerId) {
                    if (this.UpperRight != ownerId && this.UpperLeft == ownerId)
                        return 81; // one corner
                    if (this.UpperLeft != ownerId && this.UpperRight == ownerId)
                        return 86; // one corner
                    if (this.UpperRight != ownerId && this.UpperLeft != ownerId)
                        return 100; // 2 corners 
                    return 41;
                }
            }
            ...
            if (edges == 4) {
                ...
                if (corners == 2) {
                    ...
                    if (this.LowerRight != ownerId && this.UpperRight != ownerId)
                        return 23;
                    if (this.UpperLeft != ownerId && this.LowerRight != ownerId)
                        return 24;
                    if (this.LowerLeft != ownerId && this.UpperRight != ownerId)
                        return 25;
                }
                if (corners == 3) {
                    if (this.UpperLeft != ownerId)
        ...
        return OwnerNeigboursInfo;
    }());
    StarMapModule.OwnerNeigboursInfo = OwnerNeigboursInfo;
    ...
    StarMapModule.Coordinate = Coordinate;
    var Box = /** @class */ (function () {
        ...
        function Box(lowerLeft, width) {
            ...
            this.Influence = []; //user->influence array
            this.OwnerId = 0;
            this.OwnerImagesType = 0; //0 = now image. number does describe the form of the border, but not its color
            //create borders depending on user diplomatic status: save diplomatic status per tile, then create borders
            this.DiplomaticId = 0; //war, hostile. peace etc
            this.DiplomaticImagesType = 0; //0 = now image. number does describe the form of the border, but not its color
            ...
            this.EntityId = 0;
            this.EntityImagesType = 0; //0 = now image. number does describe the form of the border, but not its color
            this.starData = null; //if a star or other object with impuls-space exists (in contrast to hyper space)
            this.drawArrow = false; //if true, draw an arrow pointing to this Tile
            this.lowerLeft = lowerLeft;
            this.width = width;
        }
        Box.prototype.OwnershipText = function () {
            if (!this.OwnerId)
                return "";
            var Text = "";
        ...
        Box.prototype.OwnerNeigboursGet = function () {
            if (this.OnwerNeighbour == null)
                this.OnwerNeighbour = new OwnerNeigboursInfo();
            return this.OnwerNeighbour;
        };
        Box.prototype.DiplomaticNeigboursGet = function () {
            if (this.DiplomaticNeighbour == null)
                this.DiplomaticNeighbour = new OwnerNeigboursInfo();
            return this.DiplomaticNeighbour;
        };
        Box.prototype.EntityNeighbourGet = function () {
            if (this.EntityNeighbour == null)
                this.EntityNeighbour = new OwnerNeigboursInfo();
            return this.EntityNeighbour;
        };
        Box.prototype.AddInfluence = function (userId, value) {
        ...
        Box.prototype.GetNeighbours = function () {
            var Neighbours = [];
    ...
    StarMapModule.GetField = GetField;
    var StarMap = /** @class */ (function () {
        function StarMap(region) {
            //up to 4 boxes with width=1 are included
            this.fields = [];
            this.region = region;
        }
        StarMap.prototype.insert = function (field) {
        ...
        StarMap.prototype.insertStar = function (field, imageId, id) {
            if (id === void 0) { id = 0; }
            ...
            if (!existingField.MovementCost)
                existingField.MovementCost = 10;
        };
        StarMap.prototype.getNeighbourFields = function (field, distance, overallRings) {
            var ret = [];
            var existingField = this.queryOrInsert(field);
        ...
        StarMap.prototype.makeNeighbourPath = function (neighbourBox) {
        ...
        StarMap.prototype.getNeighbourFieldsPath = function (field, distance) {
            var ret = [];
            var existingField = this.queryOrInsert(field);
            var leftX = existingField.lowerLeft.x - distance;
            var upY = existingField.lowerLeft.y - distance;
        ...
        StarMap.prototype.insertOwnership = function (field, ownerId) {
        ...
        */
        ...
        StarMap.prototype.subdivide = function () {
        ...
        StarMap.prototype.queryRange = function (range) {
            var includedBoxes = [];
            ...
            for (var i = 0; i < this.fields.length; i++) {
                if (this.fields[i].intersectsBox(range))
                    includedBoxes.push(this.fields[i]);
            }
        ...
        StarMap.prototype.queryAll = function () {
        ...
        StarMap.prototype.queryOrInsert = function (field) {
            if (field.width != 1)
                return null;
    ...
    StarMapModule.StarMap = StarMap;
    function InfluenceToRingNo(Influence) {
    ...
    StarMapModule.InfluenceToRingNo = InfluenceToRingNo;
    function applyInfluence(field, influence, userId) {
    ...
    /// <returns></returns>
    function RingToMinInfluence(Ring) {
        var MinInfluence = 1;
        for (var index = 0; index < mainObject.influenceRings.length; ++index) {
            var InfluenceRing = mainObject.influenceRings[index];
    ...
    StarMapModule.RingToMinInfluence = RingToMinInfluence;
    ...
    StarMapModule.ReCreateBorders = ReCreateBorders;
    function AddColonyOwnership(colony) {
    ...
    StarMapModule.AddColonyOwnership = AddColonyOwnership;
    ...
    function SetFieldOwners() {
        ...
        for (var i = 0; i < allFields.length; i++) {
            ...
            for (var j = 0; j < fieldToSet.Influence.length; j++) {
                if (fieldToSet.Influence[j] == null)
                    continue;
    ...
    function SetFieldVisibleOwnership() {
        var allFields = StarMapModule.starMap.queryAll();
        for (var i = 0; i < allFields.length; i++) {
    ...
    function CreateNeutralZone() {
        ...
        for (var i = 0; i < allFields.length; i++) {
            if (allFields[i].OwnerId != 0) {
                ...
                //get the 4 neighbours, each of these will be checked
                for (var j = 0; j < Neighbours.length; j++) {
                    if (Neighbours[j].OwnerId == 0 || Neighbours[j].OwnerId == currentOwner)
                        continue;
                    //skip if neighbour is allied or has a pact
                    //allied:
                    if (allFields[i].EntityId == Neighbours[j].EntityId)
                        continue;
                    //pact or better:
        ...
        for (var i = 0; i < allFields.length; i++) {
            if (allFields[i].OwnerId != 0) {
                ...
                for (var j = 0; j < Neighbours.length; j++) {
                    ...
                    if (Neighbours[j].OwnerId == currentOwner || allFields[i].EntityId == Neighbours[j].EntityId)
                        FoundNeighbour = true;
                    //pact or better:
                    //fetch relation between the two users:
                    /*
...
})(StarMapModule || (StarMapModule = {}));
//# sourceMappingURL=StarMapQuadTree.js.map