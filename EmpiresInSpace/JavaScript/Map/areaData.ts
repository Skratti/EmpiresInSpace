...
class AreaSpecifications extends SpaceObject {
    ...
    {          
        ...
        if (!this.ColonyMap) mainObject.currentColony = null;         

        mainObject.currentArea = this;
        currentMap = this.tilemap;
        if (this.ColonyMap) currentMap = this.ColonyMap;
...
class GalaxyData extends AreaSpecifications {
    ...
    useSolarSystems: boolean = false; //Todo: should be set first when client is started - perhaps using a websocket 
...
class StarData extends AreaSpecifications implements Colonizable {
   
    elementsInArea: SurfaceField[] = []; // PlanetData is added here
    shipsInArea: Ships.Ship[] = [];
    tilemap: TilemapModule.SolarSystemMap; //= new SolarSystemMap(this);    
    hasSubElements = true; //true if solarSystem or colony is present
    size = 100;
...
class PlanetData extends AreaSpecifications implements Colonizable {
...
class SurfaceField extends AreaSpecifications {
    ...
    constructor(Id, public parentArea: StarData | PlanetData) {
        super(Id);                
    }
    ...
    update(XMLStar)
    {
        ...
        */
        ...
        if ((<Colonizable>this.parentArea).colony != null) {
