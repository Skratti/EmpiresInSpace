var BuildModulesForm;
(function (BuildModulesForm) {
    var popup;
    var body;
    var tradeDiv2;
    function runTemplate() {
        ...
        BuildModulesForm.DesignerContainer = BuildModulesForm.windowHandle.element;
        //Add Caption
        var panelHeader = $('.relPopupHeader', BuildModulesForm.DesignerContainer);
        var caption = $('<h2/>', { text: i18n.label(1025) }); //Build Ship Modules
        panelHeader.append(caption);
        //Panel Layout
        var panelBody = $('.relPopupBody', BuildModulesForm.DesignerContainer);
    ...
    BuildModulesForm.runTemplate = runTemplate;
    function OpenTradeForm() {
        BuildModulesForm.windowHandle = ElementGenerator.MainPanel();
        popup = BuildModulesForm.windowHandle.element;
        body = $('.relPopupBody', popup);
        BuildModulesForm.runTemplate();
    }
    BuildModulesForm.OpenTradeForm = OpenTradeForm;
    // Create the various div elements 
    function PopulateCreateTradeForm(tradeDiv2) {
        ...
        TradeTradeAreaCancel.click(function (e) { e.stopPropagation(); BuildModulesForm.BuildModulesHelper.ClearClick(); });
        TradeTradeAreaCreate.click(function (e) { e.stopPropagation(); BuildModulesForm.BuildModulesHelper.BuildModulesClick(); });
        //goods selector page
        ...
        TradeGroupSelectButton1.click(function (e) { e.stopPropagation(); BuildModulesForm.BuildModulesHelper.SetSelectedDemandGoodType(TradeOffersModule.DemandGoodType.I); });
        if (BuildModulesForm.BuildModulesHelper.CheckModuleLevelAvailable(TradeOffersModule.DemandGoodType.II)) {
            ...
            TradeGroupSelectButton2.click(function (e) { e.stopPropagation(); BuildModulesForm.BuildModulesHelper.SetSelectedDemandGoodType(TradeOffersModule.DemandGoodType.II); });
        }
        if (BuildModulesForm.BuildModulesHelper.CheckModuleLevelAvailable(TradeOffersModule.DemandGoodType.III)) {
            ...
            TradeGroupSelectButton3.click(function (e) { e.stopPropagation(); BuildModulesForm.BuildModulesHelper.SetSelectedDemandGoodType(TradeOffersModule.DemandGoodType.III); });
        }
        if (BuildModulesForm.BuildModulesHelper.CheckModuleLevelAvailable(TradeOffersModule.DemandGoodType.SI)) {
            ...
            TradeGroupSelectButtonS1.click(function (e) { e.stopPropagation(); BuildModulesForm.BuildModulesHelper.SetSelectedDemandGoodType(TradeOffersModule.DemandGoodType.SI); });
        }
        if (BuildModulesForm.BuildModulesHelper.CheckModuleLevelAvailable(TradeOffersModule.DemandGoodType.SII)) {
            ...
            TradeGroupSelectButtonS2.click(function (e) { e.stopPropagation(); BuildModulesForm.BuildModulesHelper.SetSelectedDemandGoodType(TradeOffersModule.DemandGoodType.SII); });
        }
        tradeDiv2.append(ColonyGoods);
        tradeDiv2.append(TradeTradeArea);
        tradeDiv2.append(TradeGoodsSelect);
    }
    BuildModulesForm.PopulateCreateTradeForm = PopulateCreateTradeForm;
    var BuildModules = /** @class */ (function () {
        ...
        //colony cargo 
        BuildModules.prototype.RefreshTradeSelectedShip = function () {
            ...
            for (var currentGoodsIndex = 0; currentGoodsIndex < mainObject.goods.length; currentGoodsIndex++) {
                if (!mainObject.currentColony.goods[currentGoodsIndex])
                    continue;
                if (mainObject.goods[currentGoodsIndex].goodsType == 3)
                    continue;
            ...
            for (var currentGoodsIndex = 0; currentGoodsIndex < this.GoodsLeftForTrade.length; currentGoodsIndex++) {
                if (!this.GoodsLeftForTrade[currentGoodsIndex])
                    continue;
        ...
        BuildModules.prototype.BindDemandGoodClick = function (goodId, div, addToDemand) {
            var _this = this;
            div.bind("mousedown touchstart", function (e) { e.stopPropagation(); _this.DemandGoodClick(goodId, addToDemand); });
        };
        /*
        ...
        */
        BuildModules.prototype.CheckModuleLevelAvailable = function (levelToCheck) {
            for (var i = 0; i < mainObject.goods.length; i++) {
                ...
                if (!Shipmodule)
                    continue;
                if (levelToCheck == TradeOffersModule.DemandGoodType.I && Shipmodule.level != 1)
                    continue;
                if (levelToCheck == TradeOffersModule.DemandGoodType.II && Shipmodule.level != 2)
                    continue;
                if (levelToCheck == TradeOffersModule.DemandGoodType.III && Shipmodule.level != 3)
                    continue;
                if (levelToCheck == TradeOffersModule.DemandGoodType.SI && Shipmodule.level != 4)
                    continue;
                if (levelToCheck == TradeOffersModule.DemandGoodType.SII && Shipmodule.level != 5)
                    continue;
                if (!PlayerData.modulesAvailable(Shipmodule.id))
                    continue;
                return true;
            }
            return false;
        ...
        //goods selector page
        BuildModules.prototype.RefreshCreateModulesSelect = function () {
            ...
            for (var i = 0; i < mainObject.goods.length; i++) {
                ...
                if (!Shipmodule)
                    continue;
                if (this.SelectedDemandGoodType == TradeOffersModule.DemandGoodType.I && Shipmodule.level != 1)
                    continue;
                if (this.SelectedDemandGoodType == TradeOffersModule.DemandGoodType.II && Shipmodule.level != 2)
                    continue;
                if (this.SelectedDemandGoodType == TradeOffersModule.DemandGoodType.III && Shipmodule.level != 3)
                    continue;
                if (this.SelectedDemandGoodType == TradeOffersModule.DemandGoodType.SI && Shipmodule.level != 4)
                    continue;
                if (this.SelectedDemandGoodType == TradeOffersModule.DemandGoodType.SII && Shipmodule.level != 5)
                    continue;
                if (!PlayerData.modulesAvailable(Shipmodule.id))
                    continue;
        ...
        BuildModules.prototype.RefreshOfferSection = function () {
            ...
            goodsOverflow.append(goodsContainer);
            for (var currentGoodsIndex = 0; currentGoodsIndex < mainObject.goods.length; currentGoodsIndex++) {
                if (!this.OfferedGoods[currentGoodsIndex])
                    continue;
                if (mainObject.goods[currentGoodsIndex].goodsType == 3)
                    continue;
        ...
        BuildModules.prototype.RefreshDemandSection = function () {
            //empty and construct the goods area:
            ...
            goodsOverflow.append(goodsContainer);
            for (var currentGoodsIndex = 0; currentGoodsIndex < mainObject.goods.length; currentGoodsIndex++) {
                if (!this.DemandedGoods[currentGoodsIndex])
                    continue;
                if (mainObject.goods[currentGoodsIndex].goodsType == 3)
                    continue;
        ...
        BuildModules.prototype.DemandGoodClick = function (goodId, addToDemand) {
            ...
            for (var goodsIndex = 0; goodsIndex < Module.costs.length; goodsIndex++) {
                if (Module.costs[goodsIndex] == null)
                    continue;
            ...
            if (this.DemandedGoods[goodId] == null || this.DemandedGoods[goodId] == 0)
                ...
                this.DemandedGoods[goodId] += ToTransfer;
            if (this.DemandedGoods[goodId] > 9999)
                this.DemandedGoods[goodId] = 9999;
            if (this.DemandedGoods[goodId] < 0)
                this.DemandedGoods[goodId] = 0;
            this.RefreshOfferSection();
            this.RefreshDemandSection();
            this.RefreshTradeSelectedShip();
            this.RefreshCreateModulesSelect();
        };
        BuildModules.prototype.SetSelectedTransferAmount = function (newValue) {
        ...
        BuildModules.prototype.SetSelectedDemandGoodType = function (newValue) {
        ...
        BuildModules.prototype.ClearClick = function () {
            this.OfferedGoods = [];
            this.DemandedGoods = [];
            this.RefreshTradeSelectedShip();
            this.RefreshOfferSection();
        ...
        BuildModules.prototype.BuildModulesClick = function () {
            ...
            for (var i = 0; !changeexists && i < this.OfferedGoods.length; i++) {
                if (this.OfferedGoods[i] == null || this.OfferedGoods[i] == 0)
                    continue;
                if (mainObject.goods[i] == null)
            ...
            for (var i = 0; !changeexists && i < this.DemandedGoods.length; i++) {
                if (this.DemandedGoods[i] == null || this.DemandedGoods[i] == 0)
                    continue;
                if (mainObject.goods[i] == null)
                    continue; //should never occur...               
                if (mainObject.goods[i].goodsType == 3)
        ...
        BuildModules.prototype.MakeJSON = function () {
        ...
        BuildModules.prototype.showCreateTrade = function () {
            this.SelectedTransferAmount = TradeOffersModule.TransferAmount.One;
            this.SelectedDemandGoodType = TradeOffersModule.DemandGoodType.I;
            this.RefreshTradeSelectedShip();
            this.RefreshCreateModulesSelect();
        ...
        BuildModules.prototype.BuildAndClose = function () {
    ...
    BuildModulesForm.BuildModules = BuildModules;
    function NewColonyStock(e) {
        if (e) {
            BuildModulesForm.BuildModulesHelper.BuildAndClose();
        }
    }
    BuildModulesForm.NewColonyStock = NewColonyStock;
    function SendBuildModules(json) {
        $.connection.spaceHub.invoke("BuildModules", json).done(function (e) {
            BuildModulesForm.NewColonyStock(e);
        });
    }
    BuildModulesForm.SendBuildModules = SendBuildModules;
    BuildModulesForm.BuildModulesHelper = new BuildModules();
})(BuildModulesForm || (BuildModulesForm = {}));
//# sourceMappingURL=BuildModulesForm.js.map