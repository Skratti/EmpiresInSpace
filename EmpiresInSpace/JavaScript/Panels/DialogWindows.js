...
(function (DialogWindows) {
    DialogWindows.DetailsWindow = null;
    DialogWindows.RearchDetailsResearch = null;
    var DialogWindowTypeEnum;
    (function (DialogWindowTypeEnum) {
        DialogWindowTypeEnum[DialogWindowTypeEnum["Research"] = 0] = "Research";
        DialogWindowTypeEnum[DialogWindowTypeEnum["Building"] = 1] = "Building";
        DialogWindowTypeEnum[DialogWindowTypeEnum["ColonyBuilding"] = 2] = "ColonyBuilding";
        DialogWindowTypeEnum[DialogWindowTypeEnum["ShipModule"] = 3] = "ShipModule";
        DialogWindowTypeEnum[DialogWindowTypeEnum["ShipHull"] = 4] = "ShipHull";
        DialogWindowTypeEnum[DialogWindowTypeEnum["Goods"] = 5] = "Goods";
        DialogWindowTypeEnum[DialogWindowTypeEnum["SurfaceField"] = 6] = "SurfaceField";
        DialogWindowTypeEnum[DialogWindowTypeEnum["PlanetType"] = 7] = "PlanetType";
    })(DialogWindowTypeEnum = DialogWindows.DialogWindowTypeEnum || (DialogWindows.DialogWindowTypeEnum = {}));
    function UpdateDetails(objectToShow, callBack) {
        if (callBack === void 0) { callBack = null; }
        switch (objectToShow.DialogWindowType()) {
            case DialogWindowTypeEnum.Research:
                DialogWindows.YesButtonCallback = function (e) { BaseDataModule.DoResearch(DialogWindows.RearchDetailsResearch, callBack); };
                DialogWindows.NoButtonCallback = function (e) { };
                DialogWindows.RearchDetailsResearch = objectToShow;
                UpdateResearchDetail(objectToShow);
                break;
            case DialogWindowTypeEnum.Building:
                DialogWindows.YesButtonCallback = function (e) { };
                DialogWindows.NoButtonCallback = function (e) { };
                DialogWindows.Building = objectToShow;
                UpdateBuildingDetail(objectToShow);
                break;
            case DialogWindowTypeEnum.ColonyBuilding:
                DialogWindows.YesButtonCallback = function (e) { };
                DialogWindows.NoButtonCallback = function (e) { DialogWindows.ColonyBuildingSelfDestruct(); };
                DialogWindows.Building = mainObject.buildings[objectToShow.buildingId];
                DialogWindows.ColonyBuilding = objectToShow;
                UpdateBuildingDetail(DialogWindows.Building);
                UpdateColonyBuildingDetail(objectToShow);
                break;
        }
    }
    function ShowDetails(objectToShow, callBack) {
        if (callBack === void 0) { callBack = null; }
        if (DialogWindows.DetailsWindow == null) {
            DialogWindows.DetailsWindow = ElementGenerator.Window(null, function (e) { DialogWindows.DetailsWindow = null; });
            $('.yesButton', DialogWindows.DetailsWindow.element).click(function (e) { DialogWindows.YesButtonCallback(e); });
            $('.noButton', DialogWindows.DetailsWindow.element).click(function (e) { DialogWindows.NoButtonCallback(e); });
        }
        else {
            $('.relPopupHeader h2', DialogWindows.DetailsWindow.element).remove();
            $('.relPopupBody', DialogWindows.DetailsWindow.element).empty();
        }
        UpdateDetails(objectToShow, callBack);
        if (DialogWindows.DetailsWindow == null) {
            ElementGenerator.setLeftTopPosition(DialogWindows.DetailsWindow.element);
        }
    }
    DialogWindows.ShowDetails = ShowDetails;
    function ShowPlayerResearchDetail(playerResearch, afterResearchCallback) {
        if (afterResearchCallback === void 0) { afterResearchCallback = null; }
        var Research = BaseDataModule.researches[playerResearch.id];
        ShowDetails(Research, afterResearchCallback);
    }
    DialogWindows.ShowPlayerResearchDetail = ShowPlayerResearchDetail;
    ...
    */
    ...
    function ResearchClicked(research, ResearchBox) {
        ResearchBox.click(function (e) { DialogWindows.ShowDetails(research); });
    }
    function UpdateResearchDetail(Research) {
        //Left side:
        // Costs
        ...
        var DetailPanel = DialogWindows.DetailsWindow.element;
        ...
        for (var j = 0; j < connections.length; j++) {
            if (connections[j].targetType != ObjectTypes.Research)
                continue;
            var PreResearch = BaseDataModule.getRelationObject(connections[j].sourceType, connections[j].sourceId);
            if (PreResearch.NeedsAdditionalSpecResearch())
                continue;
        ...
        for (var j = 0; j < connections.length; j++) {
            if (connections[j].targetType != ObjectTypes.Research)
                continue;
            var FollowingResearch = BaseDataModule.getRelationObject(connections[j].targetType, connections[j].targetId);
            if (FollowingResearch.NeedsAdditionalSpecResearch())
                continue;
            var ResearchBox = $('<div class="ResearchBox">');
        ...
        for (var j = 0; j < connections.length; j++) {
            ...
            if (connections[j].targetType == ObjectTypes.Research || connections[j].targetType == ObjectTypes.Quest || connections[j].targetType == ObjectTypes.Good)
                continue;
    ...
    function UpdateBuildingDetail(building) {
        var detailPanel = DialogWindows.DetailsWindow.element;
        $('.relPopupPanel', detailPanel).addClass("NoHover");
        var body = $('.relPopupBody', detailPanel);
        ...
        for (var goodsIndex = 0; goodsIndex < building.costs.length; goodsIndex++) {
            if (building.costs[goodsIndex] == null)
                continue;
        ...
        for (var goodsIndex = 0; goodsIndex < building.production.length; goodsIndex++) {
            if (building.production[goodsIndex] == null)
                continue;
            if (goodsIndex == 8)
                continue;
        ...
        for (var tileIndex = 0; tileIndex < mainObject.surfaceTiles.length; tileIndex++) {
            if (mainObject.surfaceTiles[tileIndex] == null)
                continue;
            if (mainObject.surfaceTileBuildings[tileIndex] == null)
                continue;
            if (mainObject.surfaceTileBuildings[tileIndex][building.id] == null)
                continue;
    ...
    function RazeBuildingDone(e) {
        if (e) {
            var currentColony = mainObject.coloniesById[DialogWindows.ColonyBuilding.colonyId];
            DialogWindows.ColonyBuilding.deleteBuilding();
        ...
        DialogWindows.DetailsWindow.remove();
        DialogWindows.DetailsWindow = null;
    }
    DialogWindows.RazeBuildingDone = RazeBuildingDone;
    function ColonyBuildingSelfDestruct() {
        var currentColony = mainObject.coloniesById[DialogWindows.ColonyBuilding.colonyId];
        $.connection.spaceHub.invoke("RazeBuilding", DialogWindows.ColonyBuilding.colonyId, DialogWindows.ColonyBuilding.id).done(function (e) {
            DialogWindows.RazeBuildingDone(e);
        });
    }
    DialogWindows.ColonyBuildingSelfDestruct = ColonyBuildingSelfDestruct;
    function UpdateColonyBuildingDetail(colonyBuilding) {
        var detailPanel = DialogWindows.DetailsWindow.element;
        $('.relPopupPanel', detailPanel).addClass("NoHover");
        ...
        if (colonyBuilding != null) {
            ...
            activeCheckBox.click(function (e) {
    ...
    function showBuildingDetail(building, colonyBuilding) {
        ...
        $('.yesButton span', detailPanel).text('OK');
        detailPanel.appendTo("body"); //attach to the <body> element
        $('.yesButton', detailPanel).click(function (e) { detailPanel.remove(); });
        ElementGenerator.setLeftTopPosition(detailPanel);
    }
    DialogWindows.showBuildingDetail = showBuildingDetail;
})(DialogWindows || (DialogWindows = {}));
//# sourceMappingURL=DialogWindows.js.map