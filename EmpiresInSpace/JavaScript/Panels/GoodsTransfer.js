...
(function (TransferModule) {
    ...
    (function (TransferAmount) {
        ...
        TransferAmount[TransferAmount["All"] = 100000] = "All";
    })(TransferAmount = TransferModule.TransferAmount || (TransferModule.TransferAmount = {}));
    var TransferPanel = /** @class */ (function () {
        function TransferPanel() {
            this.IsOpen = false;
            this.HasChanges = false;
            this.TransferredGoods = []; //goodid and transferamount. Positiv is sender to target, negativ is target to sender
            this.AmountToTransfer = TransferAmount.All;
        ...
        */
        TransferPanel.prototype.SetTransferAmount = function (newValue) {
        ...
        TransferPanel.prototype.ShowTargetStorage = function () {
            ...
            for (var currentGoodsIndex = 0; currentGoodsIndex < mainObject.goods.length; currentGoodsIndex++) {
                if (this.Target.goods[currentGoodsIndex] == null && this.TransferredGoods[currentGoodsIndex] == null)
                    continue;
                if (mainObject.goods[currentGoodsIndex].goodsType == 3)
                    continue;
                var amountInStore = 0;
                if (this.Target.goods[currentGoodsIndex] != null)
                    amountInStore = this.Target.goods[currentGoodsIndex];
        ...
        TransferPanel.prototype.RefreshShipList = function () {
            ...
            if (mainObject.currentColony == null) {
                if (this.tile.astronomicalObject != null) {
                    if (this.tile.astronomicalObject instanceof PlanetData) {
                        var planet = this.tile.astronomicalObject;
            ...
            for (var i = 0; i < this.tile.ships.length; i++) {
                if (this.tile.ships[i] == null)
                    continue;
                if (this.tile.ships[i] == this.Sender)
                    continue;
        ...
        TransferPanel.prototype.RefreshTargetDetails = function () {
            this.TargetCanTransfer = true;
        ...
        TransferPanel.prototype.BindShipListShipClick = function (spaceObject, shipDiv) {
            shipDiv.bind("mousedown touchstart", function (e) { e.stopPropagation(); transferPanel.SwitchTarget(spaceObject); });
        };
        TransferPanel.prototype.ResetChanges = function () {
            this.TransferredGoods = [];
            this.HasChanges = false;
            this.ShowTargetStorage();
            DrawInterface.refreshQuickInfoGoods();
        };
        TransferPanel.prototype.ChooseInitialTarget = function () {
            if (this.Target != null)
                return;
            ...
            if (this.Target == null)
                this.Target = TrashCan;
            this.RefreshTargetDetails();
        };
        TransferPanel.prototype.SwitchTarget = function (spaceObject) {
            if (this.HasChanges)
                this.SendChanges();
        ...
        TransferPanel.prototype.OpenTransfer = function () {
            if (this.IsOpen) {
                this.CloseTransfer();
                return;
            }
            if (this.Sender == null && mainObject.currentShip != null)
                this.Sender = mainObject.currentShip;
            if (this.Sender == null && mainObject.currentColony != null)
                this.Sender = mainObject.currentColony;
        ...
        TransferPanel.prototype.CloseTransfer = function () {
            if (this.HasChanges)
                this.SendChanges();
        ...
        TransferPanel.prototype.MakeJSON = function () {
            ...
            if (this.Sender == null || this.Target == null || this.TransferredGoods.length == 0)
                return "";
            ...
            for (var i = 0; i < this.TransferredGoods.length; i++) {
                if (this.TransferredGoods[i] == null || this.TransferredGoods[i] == 0)
                    continue;
        ...
        TransferPanel.prototype.SendChanges = function () {
        ...
        TransferPanel.prototype.RecycleSelection = function (goodid, amount) {
            //if recycling action is executes, add the raw materials to storage
        ...
        TransferPanel.prototype.PendingTransfer = function (goodId) {
            if (this.TransferredGoods.length <= goodId)
                return 0;
            if (this.TransferredGoods[goodId])
                return this.TransferredGoods[goodId];
            return 0;
        };
        TransferPanel.prototype.BindTargetGoodClick = function (goodId, div) {
            div.bind("mousedown touchstart", function (e) { e.stopPropagation(); transferPanel.TargetGoodClick(goodId); });
        };
        TransferPanel.prototype.TargetGoodClick = function (goodId) {
            if (!this.TargetCanTransfer)
                return;
        ...
        TransferPanel.prototype.BindSenderGoodClick = function (goodId, div) {
            div.bind("mousedown touchstart", function (e) { e.stopPropagation(); transferPanel.SenderGoodClick(goodId); });
        };
        TransferPanel.prototype.SenderGoodClick = function (goodId) {
            if (!this.IsOpen)
                return;
            //if recycling ist the choosen target, skip all non-module goods:
            if (this.Target.id == 0 && mainObject.goods[goodId].goodsType != 2)
                return;
            this.HasChanges = true;
            var Pending = this.TransferredGoods[goodId] || 0;
        ...
        TransferPanel.prototype.CargoChange = function (spaceObject) {
            if (!(this.Sender == spaceObject || this.Target == spaceObject))
                return 0;
    ...
    TransferModule.TransferPanel = TransferPanel;
    ...
    function enableTransferButton(_spaceObject) {
        if (_spaceObject.owner != mainObject.user.id)
            return;
        var counter = 0;
        if (_spaceObject.parentArea instanceof StarData) {
            ...
            if (tile.astronomicalObject) {
                var star = tile.astronomicalObject;
                if (star.colony && star.isMainColony())
                    counter += 2;
            }
        }
    ...
    TransferModule.enableTransferButton = enableTransferButton;
    ...
    TransferModule.CreateSymbols = CreateSymbols;
})(TransferModule || (TransferModule = {}));
//# sourceMappingURL=GoodsTransfer.js.map