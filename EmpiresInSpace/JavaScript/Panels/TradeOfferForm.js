...
(function (TradeOffersModule) {
    ...
    var Designer = /** @class */ (function (_super) {
        ...
        function Designer() {
            ...
            _super.call(this, 3, 2) || this;
            _this.run();
            return _this;
        }
        Designer.prototype.run = function () {
            TradeOffersModule.windowHandle = ElementGenerator.MainPanel();
            popup = TradeOffersModule.windowHandle.element;
            body = $('.relPopupBody', popup);
            body.addClass('Trade');
            TradeOffersModule.windowHandle.setHeader(i18n.label(119));
            refreshDataAndRun();
        };
        Designer.prototype.runOld = function () {
            /*
            super.run();
    ...
    function OpenTradeForm() {
        ...
        TradeOffersModule.windowHandle.setHeader(i18n.label(119));
        refreshDataAndRun();
        TradeOffersModule.windowHandle.callbackOnRemove = function (e) { TradeOffersModule.RemoveTradeWindow(); };
    }
    TradeOffersModule.OpenTradeForm = OpenTradeForm;
    function RemoveTradeWindow() {
        TradeOffersModule.tradeOffers.forEach(function (value, index) { value.OfferElement = null; });
        TradeOffersModule.TradeListPanel.goodsAvailable = [];
    }
    TradeOffersModule.RemoveTradeWindow = RemoveTradeWindow;
    ...
    function createMinimalDiplomaticStateSelect() {
    ...
    function runTemplate() {
        ...
        TradeOffersModule.DesignerContainer = TradeOffersModule.windowHandle.element;
        //Add Caption
        var panelHeader = $('.relPopupHeader', TradeOffersModule.DesignerContainer);
        var caption = $('<h2/>', { text: i18n.label(143) });
        panelHeader.append(caption);
        //Panel Layout
        var panelBody = $('.relPopupBody', TradeOffersModule.DesignerContainer);
        ...
        TradeOffersModule.TradeListPanel.allTradesDiv = allTradesDiv1;
        //2. Tab: Offer wares (page)
        tradeDiv2 = $("<div id='tab2'/>");
        TradeOffersModule.PopulateCreateTradeForm(tradeDiv2);
        //3. Tab:: Trade ports
        //All offers (page)
        ...
        TradeOffersModule.TradeListPanel.initTradingPosts();
    ...
    TradeOffersModule.runTemplate = runTemplate;
    function TradePortsHeader() {
        //654
        var tableRow = $('<tr/>');
    ...
    function TradePortLine(_caller, tradePort) {
        var tableRow = $('<tr/>');
        //Id  - Name - Besitzer - Koordinaten - Anzahl Angebote
    ...
    function showTradePorts() {
        ...
        for (var i = 0; i < CommModule.commNodes.length; i++) {
            if (CommModule.commNodes[i] == null)
                continue;
            if (!CommModule.commNodes[i].visited)
                continue;
            if (CommModule.commNodes[i].connectionType == 4)
                continue; //skip alliances
            filteredArray.push(CommModule.commNodes[i]);
        }
        TradeOffersModule.windowHandle.createTable(tradeDiv3, filteredArray, TradePortsHeader, TradePortLine, null, 0, null, 20, false);
    ...
    */
    ...
    TradeOffersModule.close = close;
})(TradeOffersModule || (TradeOffersModule = {}));
//# sourceMappingURL=TradeOfferForm.js.map