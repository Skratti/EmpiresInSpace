...
(function (TradeOffersModule) {
    var goodsWithoutOffers = []; // goodsId - Amount
    ...
    function hasArrayNegatives(_array) {
    ...
    function sumAllTrades(_spaceObject) {
        var goodsSum = []; // goodsId - Amount
        for (var i = 0; i < TradeOffersModule.tradeOffers.length; i++) {
            ...
            if (TradeOffersModule.tradeOffers[i].creatorId != _spaceObject.id)
                continue;
            if (_spaceObject instanceof Ships.Ship && (TradeOffersModule.tradeOffers[i].objectType != 0))
                continue;
            Helpers.addGoodsToArray(goodsSum, TradeOffersModule.tradeOffers[i].offer);
        }
        return goodsSum;
    }
    function acceptTradeOffer2(tradeOffer) {
        ...
        var trader = null;
        var goodsToCheck = []; //amount on the ship. 
        if (TradeOffersModule.callingObject != null) {
            Helpers.addGoodsToArray(goodsToCheck, TradeOffersModule.callingObject.goods);
            Helpers.removeGoodsFromArrayNegatives(goodsToCheck, sumAllTrades(TradeOffersModule.callingObject));
            Helpers.removeGoodsFromArrayNegatives(goodsToCheck, tradeOffer.request);
            if (!hasArrayNegatives(goodsToCheck))
                trader = TradeOffersModule.callingObject;
        }
        if (trader == null) {
    ...
    function acceptTradeFailed(failureCode) {
    ...
    function createTableLine(tradeOffer, useFilter) {
        ...
        if (useFilter) {
            ...
            switch (TradeOffersModule.TradeListPanel.userFilter) {
                case '0':
                    break;
                case '5':
                    if (tradeOffer.userId != mainObject.user.id)
                        return null;
                    break;
                default:
                    if (mainObject.user.otherUserExists(tradeOffer.userId) && mainObject.user.otherUserFind(tradeOffer.userId).currentRelation < parseInt(TradeOffersModule.TradeListPanel.userFilter, 10))
                        return null;
                    break;
            }
            //offered
        ...
        for (var goodsIndex = 0; goodsIndex < tradeOffer.offer.length; goodsIndex++) {
            if (tradeOffer.offer[goodsIndex] == null)
                continue;
            if (offerFilterInt == goodsIndex)
                foundGoodInOffer = true;
        ...
        for (var goodsIndex = 0; goodsIndex < tradeOffer.request.length; goodsIndex++) {
            if (tradeOffer.request[goodsIndex] == null)
                continue;
            if (requestFilterInt == goodsIndex)
                foundGoodInRequest = true;
    ...
    TradeOffersModule.createTableLine = createTableLine;
    var TradeList = /** @class */ (function () {
        function TradeList() {
            this.userFilter = "0"; //all users
            this.offerFilter = "-1"; // all goods that are offered        
            this.requestFilter = "-2"; // all goods that are requested (-1 is "only acceptable"/payable)
            this.goodsAvailable = []; // all goods from the player at all locations that are not in sales offers
            this.PayoutLocationCommNodeId = null;
            this.PayoutLocationCommNode = null;
        }
        //calcs the cargoroom and offers of the player for each trading post
        TradeList.prototype.initTradingPosts = function () {
            for (var i = 0; i < CommModule.commNodes.length; i++) {
                if (typeof CommModule.commNodes[i] === 'undefined') { /*Helpers.Log("undef");*/
                    continue;
                }
                if (!CommModule.commNodes[i].visited) { /*Helpers.Log("not visited yet");*/
                    continue;
                }
                ...
                var galaxyColrRow = { col: CommModule.commNodes[i].positionX, row: CommModule.commNodes[i].positionY };
                if (!galaxyMap.tilemap.tileExist(galaxyColrRow))
                    continue;
                var galaxyTile = galaxyMap.tilemap.findCreateTile(galaxyColrRow);
                if (CommModule.commNodes[i].systemX != 0 && CommModule.commNodes[i].systemY != 0) {
                    if (galaxyTile.astronomicalObject == null)
                        continue;
                    var systemColRow = { col: CommModule.commNodes[i].systemX, row: CommModule.commNodes[i].systemY };
                    if (!galaxyTile.astronomicalObject.tilemap.tileExist(systemColRow))
                        continue;
                ...
                for (var j = 0; j < tileToCheck.ships.length; j++) {
                    if (typeof tileToCheck.ships[j] === 'undefined') {
                        continue;
                    }
                    if (tileToCheck.ships[j].owner != mainObject.user.id)
                        continue;
            ...
            for (var j = 0; j < TradeOffersModule.tradeOffers.length; j++) {
                if (typeof TradeOffersModule.tradeOffers[j] === 'undefined') {
                    continue;
                }
                if (TradeOffersModule.tradeOffers[j].userId != mainObject.user.id)
                    continue;
        ...
        TradeList.prototype.createGoodsAndPayoutSelector = function () {
            //
            ...
            for (var currentGoodsIndex = 0; currentGoodsIndex < mainObject.goods.length; currentGoodsIndex++) {
                if (!this.goodsAvailable[currentGoodsIndex])
                    continue;
            ...
            var highestCargoCapacitiyNode = null;
            for (var i = 0; i < CommModule.commNodes.length; i++) {
                if (typeof CommModule.commNodes[i] === 'undefined') { /*Helpers.Log("undef");*/
                    continue;
                }
                ...
                if (CommModule.commNodes[i].overallCargoRoom <= 0) { /*Helpers.Log("undef");*/
                    continue;
                }
        ...
        TradeList.prototype.showAllTrades = function () {
            ...
            for (var i = TradeOffersModule.tradeOffers.length; i > 0; i--) {
                ...
                (function createLineClosure(tradeOffer) {
                    ...
                    if (tableRow == null)
                        return; //if the filters permit the row, null is returned
                    if (makeGray) {
        ...
        TradeList.prototype.createFilterLine = function () {
            var _this = this;
            var tableRow = $('<tr/>');
            //Seller + Filter
            ...
            option2.val("2");
            if (this.userFilter == "2")
                option2.attr('selected', 'selected');
            option3.val("3");
            if (this.userFilter == "3")
                option3.attr('selected', 'selected');
            option4.val("4");
            if (this.userFilter == "4")
                option4.attr('selected', 'selected');
            option5.val("5");
            if (this.userFilter == "5")
                option4.attr('selected', 'selected');
            ...
            */
            selectorSeller.selectmenu({
                change: function (event, ui) {
                    _this.userFilter = ui.item.value;
                    _this.showAllTrades();
                }
            });
            //Offer + Filter 
            ...
            if (this.offerFilter == '-1')
                optionOfferAll.attr('selected', 'selected');
            ...
            for (var i = 0; i < mainObject.goods.length; i++) {
                ...
                if (this.offerFilter == i.toString())
                ...
                if (lineCount % 2 != 0)
                    newOption.attr('data-lineSwitch', 'lineSwitch');
            }
            var offerDiv = $("<div>");
            ...
            */
            selectorOffer.iconselectmenu({
                change: function (event, ui) {
                    _this.offerFilter = ui.item.value;
                    _this.showAllTrades();
                }
            ...
            if (this.requestFilter == '-2')
                optionRequestAll.attr('selected', 'selected');
            ...
            */
            ...
            for (var i = 0; i < mainObject.goods.length; i++) {
                ...
                var imageSource = mainObject.imageObjects[mainObject.goods[i].goodsObjectId].texture.src;
                var newOption = $('<option/>', { text: i18n.label(mainObject.goods[i].label) }).val(i.toString());
                if (this.requestFilter == i.toString())
                    newOption.attr('selected', 'selected');
            ...
            */
            selectorRequest.iconselectmenu({
                change: function (event, ui) {
                    _this.requestFilter = ui.item.value;
                    _this.showAllTrades();
                }
            }).iconselectmenu("menuWidget")
                .addClass("ui-menu-icons ShipModule SelectmenuHeight");
    ...
    TradeOffersModule.TradeList = TradeList;
    TradeOffersModule.TradeListPanel = new TradeList();
})(TradeOffersModule || (TradeOffersModule = {}));
//# sourceMappingURL=TradeOffersListPage.js.map