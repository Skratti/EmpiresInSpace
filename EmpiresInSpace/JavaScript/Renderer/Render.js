...
(function (Renderer) {
    ...
    var areas = []; //array of areaIds, the map to an array of THREE.Object3D objects
    ...
    var fieldSize = 64;
    var tilemapSize = 400; //the size of the game map -> 1000x1000. About every second field gets a sprite to show. This can be modified in makeFields(), e.g. to show a sprite on every single field of the tilemap
    ...
    function getAreasToHaveInScene(id) {
        ...
        var areasToHave = [];
        if (id - (areaSize + 1) >= 0)
            areasToHave.push(id - (areaSize + 1)); //upper left
        if (id - areaSize >= 0)
            areasToHave.push(id - areaSize); //upper
        if (id - (areaSize - 1) >= 0)
            areasToHave.push(id - (areaSize - 1)); //upper right
        if (id - 1 >= 0)
            areasToHave.push(id - 1); //left
            ...
            if (fields[i] != undefined) {
                for (var j = fieldGroupUpperY; j < fieldGroupUpperY + areaSize; j++) {
                    if (fields[i][j] != undefined) {
                        ...
                        if (fields[i][j] == 5) {
                            var canvasLab = makeLabelCanvas(50, 'NameOfLocation');
    ...
    function clearAndFillScene(currentAreaId) {
        ...
        for (var i = areasinScene.length - 1; i >= 0; --i) {
            ...
            if (shouldBeRemoved) {
                ...
                for (var j = 0; j < toDelete.length; j++) {
                    if (toDelete[j] == null)
                        continue;
                    Renderer.scene.remove(toDelete[j]);
                    elementsInScene--;
                }
        ...
        for (var i = 0; i < areasToHaveInScene.length; i++) {
            ...
            if (indexInareasinScene < 0) {
                ...
                for (var j = 0; j < currentFieldArea2.length; j++) {
                    if (currentFieldArea2[j] == null)
                        continue;
                    Renderer.scene.add(currentFieldArea2[j]);
                    elementsInScene++;
                }
    ...
    function addArea(areaIdToAdd) {
        ...
        var currentFieldArea = areas[areaIdToAdd];
    ...
    function init() {
        ...
        Renderer.camera = new THREE.OrthographicCamera(window.innerWidth / -2, window.innerWidth / 2, window.innerHeight / 2, window.innerHeight / -2, 1, 50);
        ...
        currentCenterAreaId = getFieldAreaId(getCameraCoordinates(Renderer.camera.position.x) + (tilemapSize / 2), getCameraCoordinates(Renderer.camera.position.z) + (tilemapSize / 2));
        clearAndFillScene(currentCenterAreaId);
        // Lights
        var ambientLight = new THREE.AmbientLight(0x404040);
        Renderer.scene.add(ambientLight);
        Renderer.renderer = new THREE.WebGLRenderer({ preserveDrawingBuffer: true });
        ...
        Renderer.renderer.domElement.parentElement.addEventListener('mousewheel', onMouseWheel, false);
        Renderer.renderer.domElement.addEventListener('DOMMouseScroll', onMouseWheel, false); // firefox
    ...
    function onWindowResize() {
        ...
        */
        Renderer.camera.left = window.innerWidth / -2;
        Renderer.camera.right = window.innerWidth / 2;
        Renderer.camera.top = window.innerHeight / 2;
        Renderer.camera.bottom = window.innerHeight / -2;
        Renderer.camera.updateProjectionMatrix();
        Renderer.renderer.setSize(window.innerWidth, window.innerHeight);
    }
    function animate() {
    ...
    function createSceneCoords(_x, _y) {
    ...
    Renderer.createSceneCoords = createSceneCoords;
    function getGameCoords() {
        var x = (Renderer.camera.position.x / fieldSize) + (tilemapSize / 2);
        var y = (Renderer.camera.position.z / fieldSize) + (tilemapSize / 2);
        return { x: x, y: y };
    }
    ...
    function setCameraPosition(coords) {
        Renderer.camera.position.x = (coords.X * fieldSize) - ((tilemapSize / 2) * fieldSize);
        Renderer.camera.position.z = (coords.Y * fieldSize) - ((tilemapSize / 2) * fieldSize);
    }
    Renderer.setCameraPosition = setCameraPosition;
    function setCameraAndRedraw(coords) {
        setCameraPosition(coords);
        currentCenterAreaId = getFieldAreaId(getCameraCoordinates(Renderer.camera.position.x) + (tilemapSize / 2), getCameraCoordinates(Renderer.camera.position.z) + (tilemapSize / 2));
        clearAndFillScene(currentCenterAreaId);
        render();
    }
    Renderer.setCameraAndRedraw = setCameraAndRedraw;
    function onMouseMove(evt) {
        ...
        var newCenterGroupId = getFieldAreaId(getCameraCoordinates(Renderer.camera.position.x) + (tilemapSize / 2), getCameraCoordinates(Renderer.camera.position.z) + (tilemapSize / 2));
    ...
    function moveCamera(deltaX, deltaY) {
        Renderer.camera.position.x -= deltaX / zoom;
        Renderer.camera.position.z -= deltaY / zoom;
    }
    function pad(num) {
        var s = "     " + num;
    ...
    function mouseCoords(evt) {
        ...
        var cameraX = (Renderer.camera.position.x / fieldSize) + (tilemapSize / 2);
        ...
        var cameraY = (Renderer.camera.position.z / fieldSize) + (tilemapSize / 2); //from camera coordinates (z) to gameCoords (y)
