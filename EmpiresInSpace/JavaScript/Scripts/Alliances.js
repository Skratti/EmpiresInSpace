...
(function (AllianceModule) {
    ...
    function runTemplate() {
        AllianceModule.allianceListWindow = ElementGenerator.MainPanel();
        //allianceListWindow = new ElementGenerator.WindowManager(null);
        AllianceModule.allianceListWindow.setHeader(i18n.label(139));
        //allianceListWindow.makeStandardSize(); 
        //allianceListWindow.prepareScrollableTable();
        panelBody = $('.relPopupBody', AllianceModule.allianceListWindow.element);
        refreshAll();
        $('#loader')[0].style.display = 'none';
        AllianceModule.allianceListWindow.SetBottom();
    }
    AllianceModule.runTemplate = runTemplate;
    function refreshAll() {
        ...
        AllianceModule.allianceListWindow.createTable(panelBody, AllianceModule.alliances, createAllAllianceHeaderWithRelation, createAllianceLine2, null, 110, null, 50, false);
        ...
        $("select", panelBody).parent().click(function (e) { e.preventDefault(); e.stopPropagation(); });
    }
    AllianceModule.refreshAll = refreshAll;
    function leaveAlliance() {
        $.ajax("Server/Alliances.aspx", {
            ...
            ;
            if (!memberCount)
                AllianceModule.alliances[mainObject.user.allianceId].deleteAlliance();
    ...
    AllianceModule.joinAlliance = joinAlliance;
    ...
    function relationChangedTowardsAlliance(relationSelector, targetUser) {
        ...
        if (newValue >= targetUser.currentRelation) {
            ...
            var confirmDiplomacyPanel = ElementGenerator.createWorsenRelationPopup(function (e) { e.preventDefault(); AllianceModule.relationChangeAccepted(newValue, targetUser, relationSelector); confirmDiplomacyPanel.remove(); }, function (e) { e.preventDefault(); confirmDiplomacyPanel.remove(); relationSelector.value = targetUser.currentRelation.toString(); $(relationSelector).selectmenu("refresh"); }, headerText, captionText, null, 0, newValue);
    ...
    function relationChangeAccepted(newValue, targetUser, relationSelector) {
        ...
        //ToDO: remove <unknown>
        var parentRow = relationSelector.parentNode.parentNode;
    ...
    AllianceModule.relationChangeAccepted = relationChangeAccepted;
    function ownAllianceInfo() {
        ...
        if (mainObject.user.allianceId) {
            ...
            ownAllianceTable.click(function () { AllianceModule.entryPoint2(mainObject.user.allianceId, AllianceModule.allianceListWindow); });
            return ownAllianceTable;
        }
        var buttonYes = $('<button/>');
        buttonYes.text(i18n.label(452)); //Allianz erstellen
        buttonYes.click(function (e) { createAlliance(); });
        buttonYes.button();
        return buttonYes;
    }
    function createAllianceButton() {
        //own alliance or create Alliance button
        var buttonYes = $('<button/>');
        buttonYes.text(i18n.label(452)); //Allianz erstellen
        buttonYes.click(function (e) { createAlliance(); });
    ...
    function createAllAllianceHeaderWithRelation() {
        var tableRow = $('<tr/>');
    ...
    function createAllianceHeader(skipDetails) {
        if (!skipDetails)
            skipDetails = false;
        var tableRow = $('<tr/>');
        //ID
    ...
    function createAllianceLine2(_caller, _alliance) {
        var tableRow = $('<tr/>');
        if (_alliance.id != mainObject.user.allianceId) {
            ...
            tableRow.click(function () { AllianceModule.entryPoint2(mainObject.user.allianceId, AllianceModule.allianceListWindow); });
        }
        ...
        if (_alliance.id != mainObject.user.allianceId) {
            ...
            */
            ...
            if (mainObject.user.allianceId == null || AllianceModule.alliances[mainObject.user.allianceId] && AllianceModule.alliances[mainObject.user.allianceId].getUserRights()) {
                ...
                selectorTargetRelation.change(function () { relationChangedTowardsAlliance((selectorTargetRelation.get()[0]), _alliance); });
    ...
    function createAllianceLine(_caller, _alliance, skipDetails) {
        if (!skipDetails)
            skipDetails = false;
        var tableRow = $('<tr/>');
        tableRow.click(function () { Helpers.Log("TR click"); AllianceModule.entryPoint2(_alliance.id, _caller); });
    ...
    var AllianceDetails = /** @class */ (function () {
        ...
        AllianceDetails.prototype.allianceDetailsOld = function (_parent) {
            ...
            this.hasAdminRights = hasAdminRights;
            $('#loader')[0].style.display = 'none';
            this.windowHandle = new ElementGenerator.WindowManager(_parent, function (e) { _this.messageTextChanged(); });
            ...
            */
            if (!this.alliance.loaded)
                this.loadAlliance();
            this.refreshAll();
            tabDiv.tabs();
            /*------------------------------------*/
        };
        AllianceDetails.prototype.allianceDetails = function (_parent) {
            var _this = this;
            var hasAdminRights = mainObject.user.allianceId && this.alliance.id == mainObject.user.allianceId && this.alliance.getUserRights() ? true : false;
            this.hasAdminRights = hasAdminRights;
            ...
            this.windowHandle.callbackOnRemove = function (e) { _this.messageTextChanged(); };
        ...
        AllianceDetails.prototype.refreshAll = function () {
        ...
        AllianceDetails.prototype.refreshDetailsPage = function () {
        ...
        AllianceDetails.prototype.createOwnAllianceMembersTableHeader = function () {
            var tableRow = $('<tr/>');
            var th = ElementGenerator.headerElement; //labels, width, noRightBorder, tooltip
        ...
        AllianceDetails.prototype.createAllianceMemberRow = function (_parent, userdata) {
            var _this = this;
            var tableRow = $("<tr/>", { "class": "trLightGrey" });
            tableRow.click(function () { DiplomacyModule.entryPoint2(userdata.id, null); });
            ...
            */
            ...
            if (!mainObject.user.allianceId || userdata.allianceId != mainObject.user.allianceId) {
                ...
                if (mainObject.user.id == userdata.id || mainObject.user.isAllianceAdmin()) {
                    (function () {
                        var __i = userdata.id;
                        $(".invite", tableRow).click(function (_event, _input) {
                            _event.preventDefault();
                            _event.stopPropagation();
                            _this.askToLeaveAlliance(_event, _input, __i);
                        });
                    })();
                }
                if (mainObject.user.id == userdata.id) {
        ...
        AllianceDetails.prototype.createOwnAllianceInviteMembersTableHeader = function () {
            var tableRow = $('<tr/>');
            var th = ElementGenerator.headerElement; //labels, width, noRightBorder, tooltip
            var thPic = ElementGenerator.headerPictureElement; //labels, width, noRightBorder, tooltip
            tableRow.append(th(442, 40)); //id 
        ...
        AllianceDetails.prototype.createAllianceInviteMemberRow = function (_parent, userdata) {
            var _this = this;
            var tableRow = $("<tr/>");
            tableRow.click(function () { DiplomacyModule.entryPoint2(userdata.id, null); });
            //ID
            ...
            (function () {
                ...
                $(".invite", tableRow).click(function (_event, _input) {
                    _event.preventDefault();
                    _event.stopPropagation();
                    _this.invitationRevoked(_event, _input, __i);
                });
            })();
            return $(tableRow);
        };
        AllianceDetails.prototype.refreshMembersPage = function () {
            var _this = this;
            this.membersPage.empty();
            //list of persons of this alliance                         
            var all = this.alliance.allMembers();
            ...
            if (this.hasAdminRights) {
                ...
                nameInput.change(function (_event, _input) {
                    ...
                    var invitedUser = parseInt(_event.currentTarget.value, 10);
                    _this.alliance.invitedUsers[invitedUser] = 1;
        ...
        AllianceDetails.prototype.invitationRevoked = function (_event, _input, invitedUser) {
            Helpers.Log(_event.currentTarget.value);
        ...
        AllianceDetails.prototype.askToLeaveAlliance = function (_event, _input, userToDismiss) {
            var _this = this;
            ...
            var windowHandle = ElementGenerator.createNoYesPopup(function (e) { e.preventDefault(); _this.leaveAlliance(userToDismiss); windowHandle.remove(); _this.refreshMembersPage(); }, function (e) { e.preventDefault(); windowHandle.remove(); }, headerText, '', null, 0);
        ...
        AllianceDetails.prototype.leaveAlliance = function (userToDismiss) {
            var postData = userToDismiss;
            ...
            if (!memberCount)
                this.alliance.deleteAlliance();
            this.refreshMembersPage();
        };
        AllianceDetails.prototype.refreshChannelsPage = function () {
            this.channelsPage.empty();
            this.channelsPage.append($("<span/>"));
        };
        AllianceDetails.prototype.refreshPoliticsPage = function () {
        ...
        AllianceDetails.prototype.createAllianceDiplomaticsHeader = function () {
            var tableRow = $('<tr/>');
            //ID
            var goodsDiv = $("<div>");
            goodsDiv.append($("<span>", { text: i18n.label(442) })); //ID
        ...
        AllianceDetails.prototype.createAllianceDiplomaticsRow = function (_parent, userRelation) {
            if (!AllianceModule.allianceExists(userRelation.targetId))
                return;
            ...
            tableRow.append(tableDataId);
            tableRow.click(function () { DiplomacyModule.entryPoint2(alliance.id, _parent); });
            //NAME
            var tableDataName = $('<td/>');
        ...
        AllianceDetails.prototype.refreshSetupPage = function () {
            var _this = this;
            ...
            nameInput.change(function (_event, _input) {
                Helpers.Log(_event.currentTarget.value);
                _this.alliance.name = _event.currentTarget.value;
        ...
        AllianceDetails.prototype.messageTextChanged = function () {
            if (!this.descriptionChanged)
                return;
            this.descriptionChanged = false;
        ...
        /*HELPER FUNCTIONS*/
        AllianceDetails.prototype.loadAllianceWorker = function (msg) {
        ...
        AllianceDetails.prototype.loadAlliance = function () {
            var _this = this;
            $.ajax("Server/Alliances.aspx", {
                "type": "POST",
                "data": {
                ...
                _this.loadAllianceWorker(msg);
            });
        };
        return AllianceDetails;
    }());
    function entryPoint2(allianceId, _parent) {
        if (allianceId) {
    ...
    AllianceModule.entryPoint2 = entryPoint2;
    new Designer();
