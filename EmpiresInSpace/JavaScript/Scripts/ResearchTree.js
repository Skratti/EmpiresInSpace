...
(function (ResearchModule) {
    ResearchModule.svgHeight = 0;
    var researchTreePanel = /** @class */ (function (_super) {
        __extends(researchTreePanel, _super);
        function researchTreePanel() {
            var _this = 
            //call super, set scriptType to 5(ResearchTree) and Id to 1 (ResearchTree)
            _super.call(this, 5, 1) || this;
            Scripts.scriptsAdmin.scripts.push(_this);
            _this.run();
            return _this;
        }
        researchTreePanel.prototype.run = function () {
            showResearchTreePanel();
        };
        return researchTreePanel;
    }(Scripts.Script));
    function makeModifierDiv(_type, amount) {
    ...
    ResearchModule.makeModifierDiv = makeModifierDiv;
    function setResearchTreeHeader() {
        ...
        $(".relPopupHeader span", ResearchModule.researchTreeContainer).addClass('researchPoints');
        $(".relPopupHeader span", ResearchModule.researchTreeContainer).text(researchHeader);
        var modifierDiv = $(".relPopupHeader div.modifiers", ResearchModule.researchTreeContainer);
        modifierDiv.empty();
    ...
    ResearchModule.setResearchTreeHeader = setResearchTreeHeader;
    function showResearchTreePanel() {
        ...
        ResearchModule.researchTreeContainer = _DesignerContainer;
        ResearchModule.researchTreeContainer.appendTo("body"); //attach to the <body> element    
        $('.relPopupPanel', ResearchModule.researchTreeContainer).addClass("mediumTransparent");
        var panelHeader = $('.relPopupHeader', ResearchModule.researchTreeContainer);
        ...
        var panelBody = $('.relPopupBody', ResearchModule.researchTreeContainer);
        ...
        var panelFooter = $('.relPopupFooter', ResearchModule.researchTreeContainer);
        ...
        */
        UpdateAll();
        $('.noButton', ResearchModule.researchTreeContainer)[0].style.display = 'none';
        $('.yesButton span', ResearchModule.researchTreeContainer).text(i18n.label(206));
        $('.yesButton', ResearchModule.researchTreeContainer).click(function (e) { ResearchModule.researchTreeContainer.remove(); });
        //$('#loader')[0].style.display = 'none';
        setResearchTreeHeader();
    }
    function tabActive(event, ui) {
        switch (ui.newTab.index()) {
            case 0:
                showResearchTreeDiv($("#tab1", ResearchModule.researchTreeContainer), 1);
                break;
            case 1:
                showResearchTreeDiv($("#tab2", ResearchModule.researchTreeContainer), 2);
                break;
            case 2:
                showResearchTreeDiv($("#tab3", ResearchModule.researchTreeContainer), 3);
                break;
            case 3:
                showResearchTreeDiv($("#tab4", ResearchModule.researchTreeContainer), 4);
                break;
        }
        showResearchTreeDiv($("#tab1", ResearchModule.researchTreeContainer), 1);
        showResearchTreeDiv($("#tab2", ResearchModule.researchTreeContainer), 2);
    ...
    ResearchModule.tabActive = tabActive;
    function UpdateAll() {
        if (ResearchModule.researchTreeContainer == null)
            return;
        showResearchTreeDiv($("#tab1", ResearchModule.researchTreeContainer), 1);
    ...
    ResearchModule.UpdateAll = UpdateAll;
    function SVG(tag) {
        return document.createElementNS('http://www.w3.org/2000/svg', tag);
    }
    function researchColor(research) {
        /*
        ...
        "gradDarkGreen"
        */
        if (PlayerData.PlayerResearchFind(research.id).isCompleted)
            return "#gradDarkGreen";
        if (PlayerData.PlayerResearchFind(research.id).researchable)
            return "#gradGreen";
        if (research.NeedsAdditionalSpecResearch())
            return "#gradRed";
        return "#gradBlue";
    ...
    function researchColor2(research) {
        if (PlayerData.PlayerResearchFind(research.id).isCompleted)
            return "#1919FF";
        if (PlayerData.PlayerResearchFind(research.id).researchable)
            return "#00FF00";
    ...
    function appendGradient(def, newId, gradientColor1, gradientColor2) {
    ...
    function addTreeResearchClickEvent(_research, _rect) {
        _rect.click(function (e) { DialogWindows.ShowPlayerResearchDetail(_research, UpdateAll); });
        //Helpers.Log("c");
    ...
    function showResearchTreeDiv(researchDiv, mode) {
        ...
        for (var i = 0; i < BaseDataModule.researches.length; i++) {
            ...
            if (BaseDataModule.researches[i].researchType !== 0 && BaseDataModule.researches[i].treeColumn == 0)
                continue;
            if (Research.NeedsAdditionalSpecResearch())
                continue;
            var fontSize = 14;
            ...
            //var color = "#101010"
            var rectElement = $(SVG("rect")); //document.createElementNS("http://www.w3.org/2000/svg", "rect");             
            ...
            */
            ...
            if (i18n.label(BaseDataModule.researches[i].label).length > 26)
            ...
            var costWidth = 10;
            if (BaseDataModule.researches[i].cost > 999)
                costWidth = 32;
            else if (BaseDataModule.researches[i].cost > 99)
                costWidth = 24;
            else if (BaseDataModule.researches[i].cost > 9)
                costWidth = 17;
            ...
            for (var j = 0; j < connections.length; j++) {
                if (connections[j].targetType != ObjectTypes.Research)
                    continue;
                var targetResearch = BaseDataModule.getRelationObject(connections[j].targetType, connections[j].targetId);
                if (targetResearch.treeColumn == 0)
                    continue; //will not happen anymore once sql data is correct
                if (targetResearch.NeedsAdditionalSpecResearch())
                    continue;
            ...
            for (var j = 0; j < connections.length; j++) {
                ...
                if (connections[j].targetType == ObjectTypes.Research || connections[j].targetType == ObjectTypes.Quest)
                    continue;
...
//# sourceMappingURL=ResearchTree.js.map