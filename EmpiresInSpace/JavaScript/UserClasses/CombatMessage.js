...
var CombatMessageModule;
(function (CombatMessageModule) {
    CombatMessageModule.allMessagesById = []; //an array of all user-messages.
    ...
    CombatMessageModule.messageCount = 0; //the overall amount of combatMessages
    var CombatSide = /** @class */ (function () {
        function CombatSide(Damage, Output, side) {
            this.Damage = Damage;
            this.Output = Output;
            this.side = side;
        }
    ...
    var CombatMessage = /** @class */ (function (_super) {
        __extends(CombatMessage, _super);
        function CombatMessage(id) {
        ...
        CombatMessage.prototype.updateText = function (XMLmessage) {
        ...
        CombatMessage.prototype.getMessageText = function () {
        ...
        /////////////// User Interface:
        CombatMessage.prototype.showMessage = function (_parent, tableRow) {
            if (!this.loaded) {
                this.getMessageText();
        ...
        CombatMessage.prototype.createMessageCheckBoxes = function (popupPanel) {
        ...
        CombatMessage.prototype.createMessagePanel = function () {
            this.CombatSides = [];
            Helpers.Log("CombatMessage.createMessagePanel()... " + this.id.toString());
            var popupWindow = ElementGenerator.createPopupWindow(4, false, null);
            var popupPanel;
            ...
            var loop = function () {
                ...
                combatRound.Output.fadeIn(combatLoopSpeed, function () {
        ...
        // or directly
        CombatMessage.prototype.updateMessagePanel = function (panel) {
            Helpers.Log("showmessage2");
        };
        CombatMessage.prototype.Header = function () {
            var labelNo = 429;
            if (this.StarId == 0 || galaxyMap.elementsInArea[this.StarId].typeId == 5000) {
                ...
                // in system:
                var star = galaxyMap.elementsInArea[this.StarId];
        ...
        CombatMessage.prototype.createDamageLines = function (startHitpoint, maxHitPoints) {
            if (startHitpoint == maxHitPoints)
                return '';
        ...
        CombatMessage.prototype.HealthbarWidth = function (maxHitPoints, currentHitpoints) {
            if (currentHitpoints == maxHitPoints)
                return 280;
            return Math.min(Math.ceil((currentHitpoints / maxHitPoints) * 280), 280);
        };
        CombatMessage.prototype.AttackerDamaged = function (amount) {
        ...
        CombatMessage.prototype.DefenderDamaged = function (amount) {
        ...
        CombatMessage.prototype.Attacker = function (attackerDiv) {
        ...
        CombatMessage.prototype.Defender = function (defenderDiv) {
        ...
        CombatMessage.prototype.Body = function () {
        ...
        CombatMessage.prototype.Body2 = function () {
            var newSide = true; //is set when the actual shooting side (defender<->attacker) switches
            var side = this.CombatRounds[0].Side; //0 = defender  -  1 (attacker) 
            var tableOpen = false; //if a html table is currently created
            var returnHTML = '';
            var overflowDiv = $("<div/>", { "class": "researchTreePanel" });
            ...
            var currentTable = $("<table/>", { "class": "combatLogTable DefenderRightSide" });
            combatRoundDiv.append(currentTable);
            var currentCombatSide = new CombatSide(0, combatRoundDiv, this.CombatRounds[0].Side);
            ...
            //finalize message:
            var bottomLine = '<div class="combatEnd" style="display:none;">';
            if (this.DefenderHitPointsRemain == 0) {
    ...
    }(MessageModule.MessageBase));
    CombatMessageModule.CombatMessage = CombatMessage;
    var CombatRound = /** @class */ (function () {
        function CombatRound() {
        }
        return CombatRound;
    }());
    CombatMessageModule.CombatRound = CombatRound;
    function CreateCombatMessageFromJSON(combatJson) {
        var message;
        ...
        var dtString = combatJson["MessageDT"];
    ...
    CombatMessageModule.CreateCombatMessageFromJSON = CreateCombatMessageFromJSON;
    function CreateCombatRoundFromJSON(combatJson) {
    ...
    CombatMessageModule.CreateCombatRoundFromJSON = CreateCombatRoundFromJSON;
    function getReceivedMessages(fromNumber, toNumber) {
        var xhttp = GetXmlHttpObject();
        xhttp.onreadystatechange = function () {
            if (xhttp.readyState == 4) {
                var jsonResponse = xhttp.responseText;
    ...
    CombatMessageModule.getReceivedMessages = getReceivedMessages;
    function receiveResult(response, fadeInMessage) {
        if (fadeInMessage === void 0) { fadeInMessage = false; }
        var combats;
        ...
        if ($(".MessagePanel").get(0)) {
            MessageModule.userInterface.showMessages(CombatMessageModule.fromNumber, CombatMessageModule.toNumber);
        }
        return combat;
    }
    CombatMessageModule.receiveResult = receiveResult;
    function messageExists(id) {
        if (CombatMessageModule.allMessagesById[parseInt(id, 10)] != null)
            return true;
        else
            return false;
    }
})(CombatMessageModule || (CombatMessageModule = {}));
