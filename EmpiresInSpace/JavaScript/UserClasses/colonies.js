...
(function (ColonyModule) {
    ...
    var ColonyBuildQueueElement = /** @class */ (function () {
        function ColonyBuildQueueElement(id) {
            this.id = id;
        }
        ColonyBuildQueueElement.prototype.update = function (XMLobject) {
        ...
        ColonyBuildQueueElement.prototype.name = function () {
        ...
        return ColonyBuildQueueElement;
    }());
    var Colony = /** @class */ (function (_super) {
        ...
        function Colony(id) {
            ...
            _this.id = id;
            _this.name = 'Galaxy';
            _this.colonyBuildQueue = [];
            _this.size = 10;
            //galaxyColRow = { col: 1, row: 1 };
            //starColRow = { col: 1, row: 1 };
            //tilemapRef; //
            //planetArea; //  reference to the area the colony resides in
            _this.planetArea = null;
            //planetAreas all planets and moons belonging to this colony
            _this.planetAreas = null;
            //parentArea: AreaSpecifications = null;
            //scanRange = 2;
            //
            _this.population = 1;
            _this.populationPoints = 1;
            _this.populationConsumation = 0;
            _this.populationFoodGain = 40000000;
            _this.populationGainTotal = 40000000;
            _this.populationHousingLost = 0;
            _this.energy = 0;
            _this.energyTotal = 0;
            _this.energyConsumation = 0;
            _this.constructionGainBuildings = 0;
            _this.constructionGain = 0;
            _this.constructionGainTotal = 0;
            _this.constructionMax = 0;
            _this.housing = 0;
            _this.housingTotal = 0;
            _this.goodsChange = []; //Production and Consumption [goodsId]->amount (of goods per turn)
            ...
            _this.researchModifier = 0;
            _this.assemblyModifier = 0;
            _this.energyModifier = 0;
            _this.housingModifier = 0;
            _this.foodModifier = 0;
            _this.productionModifier = 0;
            _this.growthModifier = 0;
            _this.productionFullModifier = 0;
            _this.foodFullModifier = 0;
            _this.BesiegedBy = 0;
            _this.TurnsOfSiege = 10;
            _this.TurnsOfRioting = 0;
            _this.MaxResistance = 0;
            _this.Influence = 0;
            _this.allowedMines = 0;
            _this.allowedFuel = 0;
            _this.allowedChemicals = 0;
            _this.planetObjectId = 24;
            _this.cargoroom = 5000;
            return _this;
        }
        Colony.prototype.GoodsModifier = function (goodsId) {
            switch (goodsId) {
        ...
        Colony.prototype.update = function (XMLobject) {
            ...
            */
            ...
            if (!galaxyMap.useSolarSystems)
                starColRow = { col: parseInt(X, 10), row: parseInt(Y, 10) };
            ...
            if (XMLobject.getElementsByTagName("starId").length > 0) {
                ...
                if (galaxyMap && typeof galaxyMap.elementsInArea[starId] !== 'undefined' && galaxyMap.elementsInArea[starId] !== null) {
                    ...
                    if (galaxyMap.elementsInArea[starId]
                        ...
                        galaxyMap.elementsInArea[starId].tilemap.map[starColRow.col][starColRow.row].astronomicalObject.colony = this;
                        galaxyMap.elementsInArea[starId].tilemap.map[starColRow.col][starColRow.row].astronomicalObject.hasSubElements = true;
                        this.planetArea = galaxyMap.elementsInArea[starId].tilemap.map[starColRow.col][starColRow.row].astronomicalObject;
            ...
            if (!mainObject.user.otherUserExists(this.owner) && !this.ownerCurrentlyChecked && this.owner != mainObject.user.id && this.owner != 0) {
                ...
                if (mainObject.currentShip == null && mainObject.currentColony == null) {
                    ...
                    if (scanningObject instanceof Ships.Ship)
            ...
            if (ColonyModule.colonyBuildings[this.id] == null)
                ColonyModule.colonyBuildings[this.id] = [];
            ColonyModule.getColonyBuildingFromXML(XMLobject);
        ...
        Colony.prototype.countBuildings = function (buildingId) {
            var counter = 0;
            for (var i = 0; i < ColonyModule.colonyBuildings[this.id].length; i++) {
                if (ColonyModule.colonyBuildings[this.id][i] == null)
                    continue;
                if (ColonyModule.colonyBuildings[this.id][i].buildingId === buildingId)
                    counter++;
            }
            return counter;
        };
        Colony.prototype.applyModifier = function (amount, modifier) {
            return Math.ceil(amount * modifier);
        };
        //calculate population Points and  Energy and their consumation
        Colony.prototype.calcColonyRessources = function () {
            //reset
            ...
            for (var i = 0; i < ColonyModule.colonyBuildings[this.id].length; i++) {
                if (ColonyModule.colonyBuildings[this.id][i] == null)
                    continue;
                if (!ColonyModule.colonyBuildings[this.id][i].isActive)
                    continue;
            ...
            for (var i = 0; i < ColonyModule.colonyBuildings[this.id].length; i++) {
                ...
                var buildingType = mainObject.buildings[ColonyModule.colonyBuildings[this.id][i].buildingId];
                var buildingProduction = buildingType.production;
                //energy and population:
                if (buildingProduction[8] != null)
                    this.populationConsumation += buildingProduction[8];
                ...
                for (var currentGoodsIndex = 0; currentGoodsIndex < buildingProduction.length; currentGoodsIndex++) {
                    if (buildingProduction[currentGoodsIndex] == null || buildingProduction[currentGoodsIndex] == 0)
                        continue;
                    if (mainObject.goods[currentGoodsIndex] == null)
                        continue; //should never occur...
                    if (mainObject.goods[currentGoodsIndex].goodsType != 1)
                        continue; //only use standard ressources, no modules etc
                    ...
                    if (this.goodsChange[currentGoodsIndex] == null)
                        this.goodsChange[currentGoodsIndex] = 0;
        ...
        Colony.prototype.openSystemMap = function () {
        ...
        Colony.prototype.selectAndCenter = function () {
        ...
        Colony.prototype.selectAndOpen = function () {
            this.planetArea.loadAndSwitchThisMap();
        };
        Colony.prototype.onOpenColonyScreen = function () {
        ...
        Colony.prototype.checkColonyBuildQueueXML = function (XMLColonyBuildQueue) {
            this.colonyBuildQueue.length = 0;
            var xmlObj = XMLColonyBuildQueue.getElementsByTagName("Queue");
            var length = xmlObj.length;
            var lactColonyId;
            for (var i = 0; i < length; i++) {
                this.createUpdateBuildQueue(xmlObj[i]);
            }
            //Helpers.Log(length + " QueueElements added");
        };
        Colony.prototype.createUpdateBuildQueue = function (XMLColonyBuildQueueElement) {
        ...
        Colony.prototype.addXMLColonyBuildQueueElement = function (XMLColonyBuildQueueElement) {
        ...
        Colony.prototype.amountCurrentlyBuild = function (buildType, buildId) {
            var length = this.colonyBuildQueue.length;
            for (var i = 0; i < length; i++) {
                if (this.colonyBuildQueue[i] == null)
                    continue;
                if (this.colonyBuildQueue[i].buildType != buildType || this.colonyBuildQueue[i].buildId != buildId)
                    continue;
                return this.colonyBuildQueue[i].targetAmount;
            }
            return 0;
        };
        Colony.prototype.CheckSiege = function () {
        ...
        Colony.prototype.refreshMainScreenStatistics = function () {
            var _this = this;
            ...
            renameColony.button();
            renameColony.click(function (e) { _this.renameColony(); });
            ...
            */
            ...
            if (this.populationPoints + this.populationConsumation >= 10)
                remainingPop.addClass('colonyPointsOK');
            else if (this.populationPoints + this.populationConsumation >= 0)
                remainingPop.addClass('colonyPointsNotOK');
            else
                remainingPop.addClass('colonyPointsMissing');
            ...
            if (popChangeN >= 0) {
                popChange.addClass('colonyPointsOK');
                popChange.text('+' + popChange.text());
            }
            else
                popChange.addClass('colonyPointsMissing');
            ...
            if (remainingE >= 10)
                remainingESpan.addClass('colonyPointsOK');
            else if (remainingE >= 0)
                remainingESpan.addClass('colonyPointsNotOK');
            else
                remainingESpan.addClass('colonyPointsMissing');
            ...
            */
            //research
            var researchs = 0;
            for (var j = 0; j < ColonyModule.colonyBuildings[this.id].length; j++) {
                if (ColonyModule.colonyBuildings[this.id][j] == null)
                    continue;
                //if (colonyBuildings[this.id][j].buildingId !== 15) continue;
                if (!ColonyModule.colonyBuildings[this.id][j].isActive || ColonyModule.colonyBuildings[this.id][j].underConstruction)
                    continue;
                //GoodId 12 stand for generated researchpoints
                if (!mainObject.buildings[ColonyModule.colonyBuildings[this.id][j].buildingId].production[12])
                    continue;
                researchs += mainObject.buildings[ColonyModule.colonyBuildings[this.id][j].buildingId].production[12];
            }
        ...
        Colony.prototype.renameColony = function () {
            var _this = this;
            ...
            $('.yesButton', newNameContainer).click(function (e) {
                ...
                $.ajax("Server/colonies.aspx", {
                    ...
                    data: {
                        "action": "renameColony",
                        "colonyId": _this.id.toString(),
                        "newName": newName
                    }
                });
                newNameContainer.remove();
                _this.name = newName.toString();
                _this.refreshMainScreenStatistics();
            });
        };
        Colony.prototype.AbandonColonyCommit = function (abandonContainer) {
            var _this = this;
            ...
            if (input.val() == this.owner.toString()) {
                $.ajax("Server/colonies.aspx", {
                    ...
                    DrawInterface.switchToArea(_this.parentArea);
        ...
        Colony.prototype.AbandonColony = function () {
            var _this = this;
            var AbandonContainer = ElementGenerator.createNoYesPopup(function (e) { e.preventDefault(); _this.AbandonColonyCommit(AbandonContainer); AbandonContainer.remove(); }, function (e) { e.preventDefault(); AbandonContainer.remove(); }, i18n.label(763), i18n.label(764).format(this.name, this.owner.toString()), null, 1);
        ...
        Colony.prototype.tooltipLine = function () {
            var line = $('<span>', { text: "----------------------" });
            line.css("line-height", "60%");
            return line;
        };
        Colony.prototype.tooltipPopulation = function (row) {
        ...
        Colony.prototype.tooltipEnergy = function (row) {
        ...
        Colony.prototype.tooltipAssembly = function (row) {
        ...
        Colony.prototype.tooltipIndustry = function (row) {
    ...
    -6% Industrie Modifikator
        ...
        Colony.prototype.tooltipResearch = function (row, researchs) {
        ...
        Colony.prototype.fixHelper = function (e, ui) {
    ...
    ColonyModule.Colony = Colony;
    function createBQHeaderLine() {
        var tableRow = $('<tr/>');
    ...
    function createBQTableLine(buildQueueElement) {
    ...
    ColonyModule.initColonies = initColonies;
    ;
    function checkColonyXML(xmlColony) {
    ...
    ColonyModule.checkColonyXML = checkColonyXML;
    function createUpdateColony(xmlColony) {
    ...
    function addColony(XMLobject) {
        ...
        if (newColony.owner == mainObject.user.id)
            newColony.addGalaxyScanrange();
        //add surface tiles
    ...
    var colonyListPanel;
    function existColonyByCoordinates(galaxyColRow, starColRow) {
        for (var i = 0; i < mainObject.colonies.length; i++) {
            ...
            if (mainObject.colonies[i].galaxyColRow.col == galaxyColRow.col
                ...
                && mainObject.colonies[i].colRow.row == starColRow.row)
                return true;
        }
        return false;
    }
    ColonyModule.existColonyByCoordinates = existColonyByCoordinates;
    function findColonyByCoordinates(galaxyColRow, starColRow) {
        for (var i = 0; i < mainObject.colonies.length; i++) {
            if (mainObject.colonies[i] == null)
                continue;
            if (mainObject.colonies[i].galaxyColRow.col == galaxyColRow.col
                ...
                && mainObject.colonies[i].starColRow.row == starColRow.row)
                return mainObject.colonies[i];
        }
        return null;
    }
    ColonyModule.findColonyByCoordinates = findColonyByCoordinates;
    ...
    ColonyModule.showColonyList = showColonyList;
    ...
    function createTableLine(_caller, colony) {
        var tableRow = $('<tr/>');
        ...
        tableDataStarGif.click(function (e) {
            e.preventDefault();
        ...
        tableDataPlanetGif.click(function (e) { e.preventDefault(); colony.openSystemMap(); _caller.remove(); });
        ...
        tableRow.click(function (e) { colony.selectAndCenter(); _caller.remove(); });
        return tableRow;
    }
