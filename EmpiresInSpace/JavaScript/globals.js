var Coords = /** @class */ (function () {
    ...
    Coords.FromField = function (field) {
        var ret = new Coords();
        ret.X = field.X;
        ret.Y = field.Y;
        return ret;
    };
    Coords.FromColRow = function (field) {
        var ret = new Coords();
        ret.X = field.col;
        ret.Y = field.row;
        return ret;
...
function stringIsInt(valueString) {
...
var ShipTemplateStatistics = /** @class */ (function () {
    function ShipTemplateStatistics() {
    }
    ShipTemplateStatistics.prototype.getFactor = function (baseValue, count, factor) {
        return Math.pow(factor, count - 1); //will lead to 1 scanner = 100%
    };
    ShipTemplateStatistics.prototype.applyFactor = function (baseValue, count, factor) {
        var factor2 = count > 1 ? this.getFactor(baseValue, count, factor) : 1;
        return count > 1 ? Math.ceil(baseValue * factor2) : baseValue;
    };
    ShipTemplateStatistics.prototype.recalcStats = function () {
        this.crew = 0;
        this.energy = 0;
        this.neededCrew = 0;
        this.neededEnergy = 0;
        ...
        for (var i = 0; i < this.modulePositions.length; i++) {
            ...
            if (this.modulePositions[i].shipmodule.scanRange > 0)
                scanners++;
            if (this.modulePositions[i].shipmodule.damagereduction > 0)
                shield++;
            if (this.modulePositions[i].shipmodule.hitpoints > 0)
                armor++;
        }
        ...
        */
        ...
        for (var i = 0; i < this.modulePositions.length; i++) {
            if (this.modulePositions[i] === undefined || this.modulePositions[i] === null)
                continue;
            if (this.modulePositions[i].shipmodule.cargoroom == 0)
                continue;
    ...
    ShipTemplateStatistics.prototype.changeModuleInTemplate = function (shipModule, factor, speedFactor) {
        if (speedFactor === void 0) { speedFactor = 1; }
        if (shipModule.crew > 0)
            this.crew += shipModule.crew * factor;
        else
            this.neededCrew -= shipModule.crew * factor;
        if (shipModule.energy > 0)
            this.energy += shipModule.energy * factor;
        else
            this.neededEnergy -= shipModule.energy * factor;
        ...
        if (shipModule.special == 1)
            this.isColonizer = true;
    };
    ShipTemplateStatistics.prototype.removeModuleAtPosition = function (targetPosition) {
        this.isSaved = false;
        for (var i = 0; i < this.modulePositions.length; i++) {
    ...
    ShipTemplateStatistics.prototype.modulePositionExists = function (posX, posY) {
        for (var i = 0; i < this.modulePositions.length; i++) {
            if (this.modulePositions[i] == null)
                continue;
    ...
    ShipTemplateStatistics.prototype.getModuleByPosition = function (posX, posY) {
        for (var i = 0; i < this.modulePositions.length; i++) {
            if (this.modulePositions[i] == null)
                continue;
            if (this.modulePositions[i].posX === posX && this.modulePositions[i].posY === posY)
    ...
    ShipTemplateStatistics.prototype.insertModuleAtPosition = function (targetPosition, shipmodule) {
        this.isSaved = false;
    ...
    ShipTemplateStatistics.prototype.isSpaceStation = function () {
        return this.shipHullId >= 199 && this.shipHullId <= 220;
    };
    ShipTemplateStatistics.prototype.CostsInBaseResources = function (Factor) {
        if (Factor === void 0) { Factor = 1; }
        var costs = [];
        var hullCosts = BaseDataModule.shipHulls[this.shipHullId].costs;
        for (var i = 0; i < hullCosts.length; i++) {
            if (hullCosts[i] == null || hullCosts[i] == 0)
                continue;
            if (costs[i] == null) {
