...
(function (DrawInterface) {
    var canvas;
    var context;
    DrawInterface.FogCanvas = document.createElement("canvas");
    DrawInterface.standardTileSize = 30;
    var backgroundSize = 600;
    DrawInterface.isScrollXEnabled = true;
    DrawInterface.isScrollYEnabled = true;
    DrawInterface.ScreenUpdate = false;
    DrawInterface.TooltipShown = false;
    var movingShipOffSet;
    //var drawLine = 0;
    var CurrentToolTip = { row: 0, col: 0 };
    function setCanvas(_canvas) {
        canvas = _canvas;
        context = canvas.getContext('2d');
        DrawInterface.FogContext = DrawInterface.FogCanvas.getContext('2d');
    }
    DrawInterface.setCanvas = setCanvas;
    function getCanvas() {
        return canvas;
    }
    DrawInterface.getCanvas = getCanvas;
    function initInterface() {
        var temp = getSize();
        screenSize.width = temp.width;
        screenSize.height = temp.height;
    }
    DrawInterface.initInterface = initInterface;
    function pixelToGrid(_x, _y) {
    ...
    DrawInterface.pixelToGrid = pixelToGrid;
    function pixelToStandardGrid(x, y) {
        var col = x;
        var row = y;
        col = Math.round(col / DrawInterface.standardTileSize);
        row = Math.round(row / DrawInterface.standardTileSize);
        return { row: row, col: col };
    }
    var fps = 0;
    ...
    var frame = 0;
    var Threshold = 30;
    function drawByRequestAnim(time) {
        if (DrawInterface.ScreenUpdate) {
            DrawInterface.ScreenUpdate = false;
    ...
    DrawInterface.drawByRequestAnim = drawByRequestAnim;
    function showFPS() {
    ...
    function drawTile(tileToDraw, displayPositionX, displayPositionY, x, y) {
        ...
        if (mainObject.imageObjects[tileToDraw.astronomicalObject.typeId] == null)
            return;
        ...
        if (tileToDraw instanceof SystemTile) {
            var xyz = tileToDraw;
            if (xyz.astronomicalObject && xyz.astronomicalObject.fieldSize === 0)
                return;
        }
        ...
        if (tileToDraw instanceof SystemTile) {
            ...
            if (tileToDraw.astronomicalObject.typeId > 45)
                PlanetOffset = -currentMap.tileSize / 2;
            ...
            context.drawImage(mainObject.imageObjects[Image.ImageId].texture, displayPositionX + PlanetOffset, displayPositionY + PlanetOffset, currentMap.tileSize * Image.Drawsize, currentMap.tileSize * Image.Drawsize);
        }
        //draw image of the star
        if (tileToDraw instanceof GalaxyTile) {
            var tileToDraw2 = tileToDraw;
        ...
        if (tileToDraw instanceof GalaxyTile && tileToDraw.astronomicalObject.typeId < 5000) {
            ...
            if (mainObject.user.showRessources) {
                ...
                systemNameRessource += i18n.label(721 + tileToDraw.astronomicalObject.ressourceId);
            }
        ...
        if (tileToDraw instanceof PlanetTile) {
            var currentPlanetTile = tileToDraw;
            ...
            if (currentPlanetTile.astronomicalObject.building != null) {
                ...
                context.drawImage(mainObject.imageObjects[mainObject.buildings[currentPlanetTile.astronomicalObject.building.buildingId].buildingObjectId].texture, displayPositionX + BuildingOffset, displayPositionY + BuildingOffset, currentMap.tileSize * 0.8, currentMap.tileSize * 0.8);
            }
            else {
                ...
                context.drawImage(z, displayPositionX, displayPositionY, currentMap.tileSize, currentMap.tileSize);
            }
            ...
            if (mainObject.selectedBuilding != null && mainObject.selectedBuilding > 0) {
                ...
                if (fieldAllowedToBuild && currentPlanetTile.astronomicalObject.building == null && currentMap.MouseOverField.col == x && currentMap.MouseOverField.row == y) {
                    var BuildingOffset = (currentMap.tileSize * 0.1);
                    context.drawImage(mainObject.imageObjects[mainObject.buildings[mainObject.selectedBuilding].buildingObjectId].texture, displayPositionX + BuildingOffset, displayPositionY + BuildingOffset, currentMap.tileSize * 0.8, currentMap.tileSize * 0.8);
                }
            }
        ...
        if (tileToDraw.astronomicalObject != null && tileToDraw.astronomicalObject.shipsInArea != null && tileToDraw.astronomicalObject.shipsInArea.length > 0) {
            for (var shipNo = 0; shipNo < tileToDraw.astronomicalObject.shipsInArea.length; shipNo++) {
                if (mainObject.currentShip && mainObject.currentShip.id == tileToDraw.astronomicalObject.shipsInArea[shipNo].id)
                    continue;
        ...
        for (var colonyCounter = 0; tileToDraw instanceof GalaxyTile && colonyCounter < mainObject.colonies.length; colonyCounter++) {
            if (mainObject.colonies[colonyCounter].galaxyColRow.col == x && mainObject.colonies[colonyCounter].galaxyColRow.row == y) {
                var currentColony = mainObject.colonies[colonyCounter];
        ...
        if (tileToDraw instanceof SystemTile) {
            if (tileToDraw.astronomicalObject.colony != null) {
                var colonyToCheck = tileToDraw.astronomicalObject.colony;
                var isMainColony = tileToDraw.astronomicalObject.isMainColony();
    ...
    function getOffset(_ship) {
        var currTime = new Date().getTime();
        if (currTime - _ship.lastMovedAt > 200)
            return { col: 0, row: 0 };
    ...
    function drawShip(tileToDraw, displayPositionX, displayPositionY, x, y) {
        var ownShip = 0;
        var enemyShip = 0;
        ...
        if (tileToDraw.ships != null && tileToDraw.ships.length > 0) {
            for (var shipNo = 0; shipNo < tileToDraw.ships.length; shipNo++) {
                if (tileToDraw.ships[shipNo].owner == mainObject.user.id) {
                    ...
                    if (mainObject.currentShip && mainObject.currentShip.id == tileToDraw.ships[shipNo].id)
                        ownShip--;
                    ownShip++;
                }
                else {
                ...
                if (mainObject.imageObjects[tileToDraw.ships[shipNo].typeId] == null)
                    return;
                ...
                if (tileToDraw.ships[shipNo].moveDirection != null) {
                    ...
                    context.drawImage(tileToDraw.ships[shipNo].objectType.texture, -(currentMap.tileSize / 2), -(currentMap.tileSize / 2), currentMap.tileSize, currentMap.tileSize);
                    context.restore(); //restore the state of canvas;
                }
                else {
                    context.drawImage(tileToDraw.ships[shipNo].objectType.texture, displayPositionX + offset.col, displayPositionY + offset.row, currentMap.tileSize, currentMap.tileSize);
                }
                if (tileToDraw.ships[shipNo].Targeted) {
                    //490
                    context.drawImage(mainObject.imageObjects[490].texture, displayPositionX + offset.col, displayPositionY + offset.row, currentMap.tileSize, currentMap.tileSize);
                }
    ...
    var ownersDrawn = [];
    function draw(sourceX, sourceY, destinationX, destinationY) {
        if (!canvas)
            return;
        ...
        */
        ...
        DrawInterface.FogContext.clearRect(0, 0, DrawInterface.FogCanvas.width, DrawInterface.FogCanvas.height);
        var Brightness = mainObject.user.scanRangeBrightness / 100;
        DrawInterface.FogContext.fillStyle = "rgba(255, 255, 255, " + Brightness + ")";
        DrawInterface.FogContext.fillRect(0, 0, DrawInterface.FogCanvas.width, DrawInterface.FogCanvas.height);
        ...
        if (currentMap.correspondingArea.Id == -1) {
            ...
            context.drawImage(DrawInterface.FogCanvas, 0, 0);
            //create a box containing more than the screen:
            ...
            for (var boxNo = 0; boxNo < visibleStarMap.length; boxNo++) {
                ...
                */
                ...
                if (imageObject && imageObject.tileSheetCount > 1) {
                    ...
                    context.drawImage(imageObject.texture, imageObject.imageSize * tileSheetPick, 0, imageObject.imageSize, imageObject.imageSize, displayPositionX - (currentMap.tileSize * imageObject.tilesOffset), displayPositionY - (currentMap.tileSize * imageObject.tilesOffset), (imageObject.tilesOffset * 2 * currentMap.tileSize) + currentMap.tileSize, //the size to be drawn has to respect its offset of its upper left corner and lower right corner.
        ...
        if (currentMap instanceof TilemapModule.PlanetMap) {
            var tileToDraw = currentMap.map[0][0];
            //var objectid: number = 10000 + ((tileToDraw.stars.parentArea.Id % 15) + 1);
            var objectid = 8000;
            var backGround = mainObject.imageObjects[objectid].texture;
        ...
        if (currentMap instanceof TilemapModule.ColonyMap) {
            ...
            for (var i = 0; i < backgroundMap.correspondingAreas.length; i++) {
                ...
                var objectid = backgroundMap.correspondingAreas[i].PlanetArea.BackgroundObjectId;
        ...
        for (var x = startCol; x < colCount; x++) {
            for (var y = startRow; y < rowCount; y++) {
                var tileToDraw = currentMap.map[x] && currentMap.map[x][y] || null;
                ...
                if (tileToDraw != null) {
                    ...
                    displayPositionX -= currentMap.scroll.x;
                    displayPositionY -= currentMap.scroll.y;
                    if (tileToDraw.drawArrow)
                        tilesWithArrow.push({ tile: tileToDraw, displayPositionX: displayPositionX, displayPositionY: displayPositionY });
                    //draw element of the map - star, planet, building...
                    if (tileToDraw.astronomicalObject != null && tileToDraw.astronomicalObject.fieldSize !== 0) {
    ...
    function BorderImageId(imagesType, diplomaticId, ownerId) {
        /*
        var z: HTMLImageElement = mainObject.imageObjects[3500].texture;
        context.drawImage(z,
            displayPositionX, displayPositionY,
        ...
        */
        ...
        if (ownerId == mainObject.user.id)
            BorderColor = 5800; // dark green
        var imageId = BorderColor + imagesType;
        return imageId;
    }
    function drawShipMovementPoints2(step, moves) {
    ...
    function drawShipMovementPath2() {
        var nexColRow;
        var endColRow = mainObject.currentShip.MoveShipEndColRow;
        ...
        if (nexColRow.col != endColRow.col || nexColRow.row != endColRow.row) {
            var Fields = Pathfinder.findPath(nexColRow.col, nexColRow.row, endColRow.col, endColRow.row, 1);
            var stepCount = Fields.length;
            if (stepCount == 0) {
                return;
            }
            if (stepCount == 1 && Fields[0].FieldCost == 0) {
                return;
            }
            ...
            if (currentMap instanceof TilemapModule.StarMap) {
                if (mainObject.currentShip) {
                    if (mainObject.currentShip.Fleet)
                        MovesRemaining = mainObject.currentShip.Fleet.starMoves;
                    else
                        MovesRemaining = mainObject.currentShip.starMoves;
                ...
                if (mainObject.currentShip) {
                    if (mainObject.currentShip.Fleet)
                        MovesRemaining = mainObject.currentShip.Fleet.sytemMoves;
                    else
                        MovesRemaining = mainObject.currentShip.sytemMoves;
    ...
    function clearShipScanAreas(displayPositionX, displayPositionY) {
        ...
        DrawInterface.FogContext.clearRect(displayPositionX, displayPositionY, currentMap.tileSize, currentMap.tileSize);
    ...
    function clearFogOfWar(displayPositionX, displayPositionY, dimension) {
        DrawInterface.FogContext.beginPath();
        DrawInterface.FogContext.clearRect(displayPositionX, displayPositionY, currentMap.tileSize * dimension, currentMap.tileSize * dimension);
        DrawInterface.FogContext.closePath();
    }
    //grey out tiles where the build is not possible
    function drawBuildingToBuiltGreyOut(displayPositionX, displayPositionY, surfaceArea) {
        if (!(surfaceArea instanceof SurfaceField))
            return false;
    ...
    function drawBuildingInactivity(displayPositionX, displayPositionY, surfaceArea) {
        if (!(surfaceArea instanceof SurfaceField))
            return;
        var surfaceArea2 = surfaceArea;
        //skip if no builidng is present or building is active
        if (surfaceArea2.building == null || surfaceArea2.building.isActive)
            return;
        context.beginPath();
        context.fillStyle = "rgba(0, 0, 0, 0.5)";
        context.fillRect(displayPositionX, displayPositionY, currentMap.tileSize, currentMap.tileSize);
    ...
    function drawTileRelationBorder(relationCode, displayPositionX, displayPositionY) {
        if (relationCode != 11) {
    ...
    function TooltipCoordinates(displayPositionX, displayPositionY, tileSize) {
    ...
    function drawCanvasTooltip(col, row, currentTime, tile) {
        if (currentMap.MouseOverField.col == col && currentMap.MouseOverField.row == row) {
            if (currentTime - currentMap.MouseMoveStartTime > 800) {
                ...
                if (CurrentToolTip.col === col && CurrentToolTip.row === row)
                    return;
                if (tile != null) {
                ...
                DrawInterface.TooltipShown = true;
    ...
    DrawInterface.drawCanvasTooltip = drawCanvasTooltip;
    ...
    function drawTileBorder(ownShip, enemyShip, displayPositionX, displayPositionY, col, row, lowestRelationCode) {
        drawTileRelationBorder(lowestRelationCode, displayPositionX, displayPositionY);
        return;
    }
    function drawTileCoordinates(x, y, movementOffSet) {
        if (movementOffSet === void 0) { movementOffSet = false; }
        var displayPositionX = currentMap.tileSize * x;
        var displayPositionY = currentMap.tileSize * y;
        displayPositionX -= currentMap.scroll.x;
        displayPositionY -= currentMap.scroll.y;
    ...
    function drawString(x, y, name, lineNumber, _bold) {
        if (lineNumber === void 0) { lineNumber = 0; }
        if (_bold === void 0) { _bold = false; }
        var displayPositionX = currentMap.tileSize * x;
        var displayPositionY = currentMap.tileSize * y;
        displayPositionX -= currentMap.scroll.x;
        displayPositionY -= currentMap.scroll.y;
    ...
    function drawOwnerString(x, y, name, lineNumber, ownerArray, ownerId) {
    ...
    function drawArrow(tileInfo) {
        var z = mainObject.imageObjects[70].texture;
    ...
    DrawInterface.checkScrollEnabling = checkScrollEnabling;
    function checkSrollOutOfBounds() {
        ...
        if (DrawInterface.isScrollXEnabled) {
        ...
        if (DrawInterface.isScrollYEnabled) {
    ...
    DrawInterface.checkSrollOutOfBounds = checkSrollOutOfBounds;
    ...
    function doResize() {
        ...
        DrawInterface.FogCanvas.width = canvas.width;
        DrawInterface.FogCanvas.height = canvas.height;
        //DrawInterface.checkMapSizeScroll();
        DrawInterface.drawAll();
    }
    DrawInterface.doResize = doResize;
    function showContextMenu() {
        Helpers.Log('showContextMenu');
    }
    DrawInterface.showContextMenu = showContextMenu;
    ...
    DrawInterface.scrollToHomePosition = scrollToHomePosition;
    function scrollToPosition(destinationX, destinationY, offset) {
    ...
    DrawInterface.scrollToPosition = scrollToPosition;
    function scrollMapToPosition(map, destinationX, destinationY, offset) {
        if (offset === void 0) { offset = 0; }
    ...
    DrawInterface.scrollMapToPosition = scrollMapToPosition;
    // checks if the map is smaller than the sreen size - it has to be centered then:
    // only for colonies
    function checkMapSizeScroll() {
    ...
    DrawInterface.checkMapSizeScroll = checkMapSizeScroll;
    function refreshNavigationBar(areaSpecification) {
        ...
        if (areaSpecification instanceof PlanetData) {
            if (areaSpecification.colony != null)
                areaName = areaSpecification.colony.name;
        }
    ...
    DrawInterface.refreshNavigationBar = refreshNavigationBar;
    ...
    DrawInterface.deselectObject = deselectObject;
    function switchToArea(areaSpecification) {
        Helpers.Log("switchToArea", Helpers.LogType.Debug);
        if (currentMap === areaSpecification.tilemap)
            return;
        deselectObject();
        areaSpecification.switchInterfaceToThisMap();
        //areaSpecification instanceof PlanetData ?
        areaSpecification.ColonyMap ?
    ...
    DrawInterface.switchToArea = switchToArea;
    function navigationEvent(liElement, areaSpecification) {
    ...
    function refreshMiddleInfoPanel() {
        ...
        if (mainObject.currentShip != null || mainObject.currentColony != null || mainObject.selectedObject != null) {
            ...
            if (mainObject.selectedObject != null && mainObject.currentColony == null)
    ...
    DrawInterface.refreshMiddleInfoPanel = refreshMiddleInfoPanel;
    //shows the middle Area with the ships that are on the same tile as the selected ship
    function clickOnColony(colony) {
        if (mainObject.currentShip != null)
    ...
    DrawInterface.clickOnColony = clickOnColony;
    DrawInterface.ShipToFleetDrag = null;
    function DotCargoColour(ship) {
        return ship.cargoHoldUsed() > 0 ? "white" : "black";
    }
    function DotHyperColour(ship) {
        if (Math.floor(ship.starMoves) == Math.floor(ship.galaxyMovesMax))
            return "hsl(200,100%,50%)"; //blue
        if (ship.starMoves > ship.galaxyMovesMax / 4)
            return "hsl(60,100%,70%)"; //yellow
        return "hsl(0,100%,60%)"; //red
        //return "hsl(120,100%,60%)"; //green
    }
    function DotSystemColour(ship) {
        if (Math.floor(ship.sytemMoves) == Math.floor(ship.systemMovesMax))
            return "hsl(200,100%,50%)"; //blue
        if (ship.sytemMoves > ship.systemMovesMax / 4)
            return "hsl(60,100%,70%)"; //yellow
        return "hsl(0,100%,60%)"; //red
    }
    function HealthbarWidth(ship) {
        if (ship.currentHitpoints == ship.hitpoints)
            return 24;
    ...
    function MakeShipWithDataDiv(ship, dots, name, bar, border) {
    ...
    function createShipDiv(ship, dots, name, bar, border) {
        if (dots === void 0) { dots = true; }
        if (name === void 0) { name = true; }
        if (bar === void 0) { bar = true; }
        if (border === void 0) { border = true; }
    ...
    DrawInterface.createShipDiv = createShipDiv;
    function DroppedShipOnShip(event, ui) {
        var target = event.target;
        var targetId = $(target).data("shipId");
        var targetShip = mainObject.shipFind(targetId);
        if (DrawInterface.ShipToFleetDrag.id == targetShip.id)
            return;
        Helpers.Log("dragging " + DrawInterface.ShipToFleetDrag.id + " to " + targetShip.id);
        if (DrawInterface.ShipToFleetDrag.Fleet != null)
            DrawInterface.ShipToFleetDrag.Fleet.RemoveShip(DrawInterface.ShipToFleetDrag);
        if (targetShip.Fleet != null) {
            targetShip.Fleet.AddShip(DrawInterface.ShipToFleetDrag);
        }
        else {
            var NewFleet = Ships.MakeFleet(true, targetShip, DrawInterface.ShipToFleetDrag);
        }
        refreshMiddleInfoPanel();
    }
    DrawInterface.DroppedShipOnShip = DroppedShipOnShip;
    function DroppedShipOnPanel(event, ui) {
        Helpers.Log("dropped on panel " + DrawInterface.ShipToFleetDrag.id);
        if (DrawInterface.ShipToFleetDrag.Fleet)
            DrawInterface.ShipToFleetDrag.Fleet.RemoveShip(DrawInterface.ShipToFleetDrag);
        refreshMiddleInfoPanel();
    }
    DrawInterface.DroppedShipOnPanel = DroppedShipOnPanel;
    function createSurfaceFieldDiv(surfaceTile) {
    ...
    DrawInterface.createSurfaceFieldDiv = createSurfaceFieldDiv;
    function createGoodsTd(goodId, amount, borderColor, signs, showAmount, colored) {
        if (showAmount === void 0) { showAmount = true; }
        if (colored === void 0) { colored = true; }
        var goodsTd = $("<td/>");
    ...
    DrawInterface.createGoodsTd = createGoodsTd;
    function createGoodsDiv(goodId, amount, borderColor, signs, showAmount, colored) {
        if (showAmount === void 0) { showAmount = true; }
        if (colored === void 0) { colored = true; }
    ...
    DrawInterface.createGoodsDiv = createGoodsDiv;
    function createGoodsImageDiv(goodId) {
    ...
    DrawInterface.createGoodsImageDiv = createGoodsImageDiv;
    function createBar(background) {
        if (background === void 0) { background = 'linear-gradient(to bottom, #77bb00 0%, #709000 100%)'; }
    ...
    DrawInterface.createBar = createBar;
    function createTextBar(LeftText, RightText, Progress, background) {
        if (background === void 0) { background = 'linear-gradient(to bottom, #77bb00 0%, #709000 100%)'; }
        var InfluenceBar;
    ...
    DrawInterface.createTextBar = createTextBar;
    function cargoHoldUsed(goodsCount, cargoRoom) {
        if (goodsCount <= 0)
            return 0;
        if (cargoRoom === -1)
            return 100;
        if (goodsCount > cargoRoom) { /*Helpers.Log('WARNING : Cargo > maxCargo');*/
            return 100;
        }
        goodsCount = Math.floor((goodsCount / cargoRoom) * 100);
        return goodsCount;
    }
    DrawInterface.cargoHoldUsed = cargoHoldUsed;
    function createCargoHoldBar(trunk, isTradebar) {
    ...
    DrawInterface.createCargoHoldBar = createCargoHoldBar;
    ...
    */
    function refreshMainScreenStatistics() {
        if (mainObject.currentColony != null)
    ...
    DrawInterface.refreshMainScreenStatistics = refreshMainScreenStatistics;
    function tooltipGoods(goodsDiv, goodsId, buildingAmount, total, colony) {
    ...
    DrawInterface.tooltipGoods = tooltipGoods;
    function refreshQuickInfoGoods() {
        //PanelController.quickInfoPanel = PanelController.QuickInfoChoice.Goods;
        var trunk; //the object which has goods to show
        var WithChange = false;
        var productionAndComsumption = [];
        var toolTipData = [];
        //get the current object
        if (mainObject.currentArea != null && mainObject.currentArea instanceof PlanetData && mainObject.currentArea.colony != null) {
            trunk = mainObject.currentArea.colony;
            productionAndComsumption = mainObject.currentArea.colony.goodsChange;
            toolTipData = mainObject.currentArea.colony.goodsChangeBuildingCount;
            WithChange = true;
        }
        else {
            if (mainObject.currentShip != undefined && mainObject.currentShip != null) {
        ...
        if (trunk === undefined)
            return;
        var trunkAndProduction = []; // sparse array that contains a flag for each good from trunk, and all goods that wil be produced are consumed 
        for (var i = 0; i < trunk.goods.length; i++) {
            if (trunk.goods[i] == null || trunk.goods[i] == 0)
                continue;
        ...
        if (productionAndComsumption.length > 0) {
            for (var i = 0; i < productionAndComsumption.length; i++) {
                if (productionAndComsumption[i] == null || productionAndComsumption[i] == 0)
                    continue;
                if (mainObject.goods[i] == null)
                    continue; //should never occur...
                if (mainObject.goods[i].goodsType == 3)
        ...
        for (var currentGoodsIndex = 0; currentGoodsIndex < mainObject.goods.length; currentGoodsIndex++) {
            if (trunkAndProduction[currentGoodsIndex] == null && transferPanel.TransferredGoods[currentGoodsIndex] == null)
                continue;
            var amountInStore = 0;
            if (trunk.goods[currentGoodsIndex] != null)
                amountInStore = trunk.goods[currentGoodsIndex];
            ...
            if (WithChange)
                $(".goodsAmount", currentGoodsDiv).addClass("WithChange");
            ...
            if (productionAndComsumption.length > 0 && productionAndComsumption[currentGoodsIndex] != null) {
                ...
                if (productionAndComsumption[currentGoodsIndex] > 0) {
                    textColorClass = "positiveChange";
                }
                if (productionAndComsumption[currentGoodsIndex] < 0) {
                    textColorClass = "negativeChange";
                }
                $(".goodsAmount", currentGoodsDiv).append($("<span/>", { "class": textColorClass, "text": changetext }));
                if (toolTipData[currentGoodsIndex] != null) {
                    tooltipGoods(currentGoodsDiv, currentGoodsIndex, toolTipData[currentGoodsIndex], productionAndComsumption[currentGoodsIndex], mainObject.currentArea.colony);
                }
            }
            else {
    ...
    DrawInterface.refreshQuickInfoGoods = refreshQuickInfoGoods;
    function addQuickMessage(textToShow, delayTime, countdown, overallTime) {
        if (delayTime === void 0) { delayTime = 12000; }
        if (countdown === void 0) { countdown = false; }
        if (overallTime === void 0) { overallTime = 2 * 60 * 1000; }
    ...
    DrawInterface.addQuickMessage = addQuickMessage;
    function HideLoader(time) {
        if (time === void 0) { time = 500; }
        setTimeout(function () { $('#loader')[0].style.display = 'none'; }, time);
    }
    DrawInterface.HideLoader = HideLoader;
