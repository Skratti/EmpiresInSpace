module FogOfWarModule {

    export var fog: FogOfWarRegion;
    ...
    export class Box {
        ...
        constructor(lowerLeft: Field, width: number = 1) {
    ...
    export class FogOfWarRegion {
        ...
        insert(field: Box) {
            ...
            if (this.A.insert(field)) { this.setIsDiscovered(); return true; }
            if (this.B.insert(field)) { this.setIsDiscovered(); return true; }
            if (this.C.insert(field)) { this.setIsDiscovered(); return true; }
            if (this.D.insert(field)) { this.setIsDiscovered(); return true; }
        ...
        deSerializeRoot(fogString: string) {
        ...
        save(server:boolean) {
            ...
            if (isClientSaved && isServerSaved) return;
        ...
        saveToServer(serializedTree: string) {
        ...
        saveToClient(serializedTree: string) {
            if (!Helpers.supportsHtmlStorage()) return;
        ...
        loadFromServer() {
            ...
            }).done((msg) => { Helpers.Log("Getting Fog from Server", Helpers.LogType.FoW);   this.deSerializeRoot(msg); });
        }
