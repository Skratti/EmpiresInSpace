module ImageCache {
    ...
    export var ImagesToLoad = 0;
    export var imagesLoaded = 0;
    ...
    export class ImageObject {
       ...
       // damage: number;
        ...
        tileSheetCount = 1;
        imageSize = 60;
        tilesOffset = 0; //0 means no offset, 1 would mean starting drawing one tile to the upper left and ending one tile offest to the lower right, meaning the tile would be three times as big

        constructor(public typeId: number, objectimageUrl: string, startUp = true, versionNo = 1) {
            ...
            if (startUp) {
                ...
                this.texture.onload = () => {
                    ...
                    if (this.typeId >= 5000 && this.typeId < 5100) {
                        this.tileSheetCount = 8;
                        this.imageSize = 240;
                        this.tilesOffset = 1.5;
                        this.moveCost = 3;
                    }
                    /*
                    if (this.rotationsGenerated > 0) {

                        this.setCanvasRotate(this.texture, 0);
                        //create the rmaining 7 tiles
                        for (var i = 1; i < this.rotationsGenerated + 1; i++) {
                            var tile = new ImageObject(this.typeId + i, null, false);
                            mainObject.imageObjects[tile.typeId] = tile;
                            tile.setCanvasRotate(this.texture, 45 * i);
                        }
                    }
                    */
                        /*
                    if (this.typeId == 3500) {
                        ...
                        //create the rmaining 7 tiles
                        for (var i = 1; i < 8; i++) {
                            var tile = new ImageObject(3500 + i, null, false);
                            mainObject.imageObjects[tile.typeId] = tile;
                            tile.setCanvasRotate(this.texture, 45 * i);
                        }
                    }
                    ...
                    if (this.typeId == 3510) {
                        ...
                        //create the rmaining 7 tiles
                        for (var i = 1; i < 8; i++) {
                            var tile = new ImageObject(3510 + i, null, false);
                            mainObject.imageObjects[tile.typeId] = tile;
                            tile.setCanvasRotate(this.texture, 45 * i);
                        }
                    }
                    ...
                    if (this.typeId == 3520) {
                        ...
                        //create the rmaining 7 tiles
                        for (var i = 1; i < 8; i++) {
                            var tile = new ImageObject(3520 + i, null, false);
                            mainObject.imageObjects[tile.typeId] = tile;
                            tile.setCanvasRotate(this.texture, 45 * i);
                        }
                    }
                    ...
                    if (this.typeId == 3530) {
                        ...
                        //create the rmaining 7 tiles
                        for (var i = 1; i < 8; i++) {
                            var tile = new ImageObject(3530 + i, null, false);
                            mainObject.imageObjects[tile.typeId] = tile;
                            tile.setCanvasRotate(this.texture, 45 * i);
                        }
                    }
                ...
                this.texture.src = "images/" + objectimageUrl + "?" + imageVersion + versionNo.toString();
            }
        ...
        isColonizable(): boolean {
            ...
            //planet earthlike
            if (this.typeId > 23 && this.typeId < 27) return true;

            //Moon earthlike 
            if (this.typeId == 34 || this.typeId == 35 || this.typeId == 36) return true;
        ...
        isMainColony(): boolean {
            //planet earthlike
            if (this.typeId > 23 && this.typeId < 32) return true;
            return false;
        }

        setCanvas(_texture: HTMLImageElement, x: number, y: number) {
            this.canvasCache = document.createElement("canvas");
            this.canvasCache.width = 60;
            this.canvasCache.height = 60;
            this.canvasCache.getContext("2d").drawImage(_texture,
                x, y, 60, 60,
                0, 0, 60, 60);
        }

        setCanvasRotate(_texture: HTMLImageElement, angle: number) {
            this.canvasCache = document.createElement("canvas");
            this.canvasCache.width = 600;
            this.canvasCache.height = 600;
            var context: CanvasRenderingContext2D = this.canvasCache.getContext("2d");
    ...
    export function checkImageLoad() {
        //bad hack: imagesLoaded > 50 --- there are atm 87 images to Load. sometimes the first one is done before the second is constructed - this would resolve to true (1 >= 1). This seems to always work, but it is really not good.
    ...
    export function addXMLobject(XMLobject: Element) {
    ...
    function generateBordersCanvas(imageId : number) {
    ...
    export function generateObjectCanvas() {
    ...
    function createTileSetPart(id: number, texture: HTMLImageElement, x: number, y: number) {
