...
/*
* mainObject.Keymap.js: bind key events to handler functions.
*
...
* the initial set of bindings for the mainObject.Keymap. The property names of this object
...
*
* See the mainObject.Keymap.keyCodeToKeyName object in this module for a list of names.
...
class Keymap {
    ...
    install(element) {
        ...
        var keymap = this;
        function handler(event) { if (keymap.isActive) return mainObject.keymap.dispatch(event, element); else return true; }
    ...
    dispatch(event, element) {
        ...
        //else
        if (event.key && event.key.substring(0, 2) !== "U+")
        {
            keyname = event.key;
            keyname = this.aliases[keyname] || keyname;
        }
            // Otherwise, use the keyCode property and the code-to-name map below
        else keyname = mainObject.keymap.keyCodeToKeyName[event.keyCode];
    ...
    normalize (keyid) {
        ...
        keyname = this.aliases[keyname] || keyname; // Is it an alias?
    ...
    aliases = {        // Map common key aliases to their "official" 
