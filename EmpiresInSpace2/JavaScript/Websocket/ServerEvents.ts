...
module ServerEventsModule {    


   

    var lastUpdatedId = 14;
    var eventsToCheck: externalGameEvent[] = [];
    var processingEvents = false;
    var readyForUpdate = true;
    var serverEventCaller: number;
    ...
    function addEvent(serverEventXML) {
        ...
        var serverEvent: externalGameEvent = { eventType: eventType, objectId: objectId, int1: int1, int2: int2, int3: int3 };
        ...
        for (var i = 0; i < eventsToCheck.length; i++) {
            switch (eventsToCheck[i].eventType) {
                case ServerEventType.TradeAccepted:
                    var trader: SpaceObject;
                    if (eventsToCheck[i].int2 == 1) trader = mainObject.coloniesById[eventsToCheck[i].int1];
                    if (eventsToCheck[i].int2 == 0) trader = mainObject.ships[eventsToCheck[i].int1];
                ...
                case ServerEventType.ColonyLost:
                    ...
                    for (var j = 0; j < ColonyModule.allBuildings.length; j++) {
                        ...
                        if (ColonyModule.allBuildings[j].colonyId == eventsToCheck[i].objectId) ColonyModule.allBuildings[j].userId = eventsToCheck[i].int1;
                    }
    ...
    export function setEventInterval() {       
        serverEventCaller = window.setInterval(
            function () { /*Helpers.Log('getEvent');*/ ServerEventsModule.getEvents(); }
            , 10 * 1000);
    }
    ...
    export function setTurnInterval() {
        setInterval(setTurnTimer, 1000);
    }


    export function setEventId(id: number) {
        lastUpdatedId = id;
    }
